(function () {
  ...
  function mensagemIndicaCpfDuplicado(texto) {
    ...
    return normalizado.includes("cpf") && (
      normalizado.includes("ja esta cadastrado")
      || normalizado.includes("nao pode cadastrar o mesmo cliente")
  ...
  function revogarPreviewFotoPendente() {
    ...
    try {
      ...
      // Mantemos o fluxo mesmo se o navegador nao permitir revogar aqui.
    }
    state.pendingFotoPreviewUrl = "";
  }

  function limparPendenciasMidia() {
  ...
  function posicionarCursorMoeda(campo) {
    ...
    const posicaoCursor = indiceVirgula >= 0 ? indiceVirgula : texto.length;
    ...
    window.requestAnimationFrame(() => {
      ...
      try {
        ...
        // Alguns estados de input nao aceitam selectionRange; mantemos apenas o valor formatado.
      }
    });
  }
  ...
  function bindCampoMoeda(id) {
    ...
    campo.addEventListener("focus", (e) => {
      posicionarCursorMoeda(e.target);
    });
  }
  ...
  function renderSecaoMidiasCliente() {
    ...
    if (el.fotoHint) {
      el.fotoHint.textContent = clienteId
        ? "Use JPG, PNG ou WEBP com ate 10 MB."
        : "Voce pode escolher a foto agora. O envio sera liberado assim que o cliente for salvo.";
    }
    ...
    if (el.documentosHint) {
      el.documentosHint.textContent = clienteId
        ? "Aceita PDF, DOC, DOCX, JPG, PNG e WEBP. Ate 12 arquivos por envio e 20 MB por documento."
        : "Voce pode selecionar os arquivos agora. Eles poderao ser enviados assim que o cliente for salvo.";
    }
            ...
            <article class="tl-clientes-docs-item">
              <div class="tl-clientes-docs-main">
                ...
                <div class="tl-clientes-docs-meta">
                  ...
                  <span>${escapeHtml(tamanhoArquivoLegivel(documento?.tamanho_bytes) || "Tamanho nao informado")}</span>
                  <span>${escapeHtml(formatarDataHoraCurta(documento?.data_hora_criacao) || "Data nao informada")}</span>
                </div>
  ...
  async function removerFotoClienteAtual() {
    ...
    if (!clienteId || !fotoAtual?.id) {
      mostrarMensagem("error", "Nao ha foto cadastrada para remover.");
      return null;
    }
  ...
  async function removerDocumentoClienteAtual(identificadorMidia) {
    ...
    if (!clienteId || !identificadorMidia) {
      mostrarMensagem("error", "Documento nao encontrado para exclusao.");
      return null;
    }
    ...
    try {
      window.sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          titulo: reserva.imovel?.titulo || "Imovel reservado",
  ...
  async function cancelarReservaCarteira(identificadorImovel) {
    ...
    const confirmado = window.confirm(`Cancelar a reserva de ${titulo}?`);
    if (!confirmado) return;

    await api(endpoint(ENDPOINTS.liberarReserva, reserva.imovel_id), "POST", {
  ...
  function renderCarteiraComercial() {
    ...
    if (!clienteId) {
      el.carteiraComercialResumo.textContent = "Salve o cliente para acompanhar simulacoes reservadas e negociacoes ligadas a ele.";
      el.carteiraComercialLista.innerHTML = '<div class="tl-clientes-lista__empty">Nenhuma simulacao reservada disponivel enquanto o cliente ainda nao foi salvo.</div>';
      return;
    }

    if (!reservas.length) {
      el.carteiraComercialResumo.textContent = "Nenhuma simulacao reservada ativa encontrada para este cliente.";
      el.carteiraComercialLista.innerHTML = '<div class="tl-clientes-lista__empty">Quando um imovel for reservado para este cliente, a simulacao reservada aparecera aqui com rastreio completo.</div>';
      return;
    }

    const totalNegociado = reservas.reduce((total, item) => total + (moedaParaNumero(item?.negociacao?.valor_total_operacao) ?? 0), 0);
    el.carteiraComercialResumo.textContent = `${reservas.length} simulacao(oes) reservada(s) ativa(s) encontradas, total negociado de ${formatarMoeda(totalNegociado)}.`;
    el.carteiraComercialLista.innerHTML = `
      <div class="tl-clientes-carteira-grid">
        ${reservas.map((reserva) => {
          const titulo = escapeHtml(reserva?.imovel?.titulo || "Imovel reservado");
          const local = escapeHtml([reserva?.imovel?.bairro, reserva?.imovel?.cidade, reserva?.imovel?.estado].filter(Boolean).join(" • ") || "Localizacao nao informada");
          const negociacao = reserva?.negociacao || {};
          const chips = [
            reserva?.status ? `Reserva ${reserva.status}` : "",
            reserva?.reservado_em ? `Criada em ${formatarDataHoraCurta(reserva.reservado_em)}` : "",
          ...
          const metricas = [
            ...
            ["Saldo pos-entrega", formatarMoeda(negociacao.saldo_pos_entrega)],
          ].filter(([, valorMetric]) => String(valorMetric || "").trim());
          const labelAcao = reserva?.simulacao_id ? "Continuar negociacao" : "Abrir no simulador";
          ...
          return `
            <article class="tl-clientes-carteira-card">
              ...
              ${reserva?.observacoes ? `<small class="tl-clientes-carteira-card__note">${escapeHtml(reserva.observacoes)}</small>` : ""}
              <div class="tl-clientes-item__actions">
                <button class="tl-imoveis-btn" type="button" data-action="abrir-reserva-simulador" data-id="${escapeHtml(reserva.imovel_id)}">${escapeHtml(labelAcao)}</button>
                <button class="tl-imoveis-btn tl-imoveis-btn--ghost" type="button" data-action="cancelar-reserva-cliente" data-id="${escapeHtml(reserva.imovel_id)}">Cancelar reserva</button>
              </div>
            </article>`;
        }).join("")}
      </div>`;
  ...
  function renderViewComposition(composicao = []) {
    if (!Array.isArray(composicao) || !composicao.length) {
      return renderViewEmpty("Nenhum membro de composicao familiar vinculado a este cadastro.");
    }

    return `
      <div class="tl-clientes-view-list tl-clientes-view-list--family">
        ${composicao.map((item) => {
          const renda = moedaVisual(obterRendaMembro(item), "Renda nao informada");
          const chips = [
  ...
  function reservaViewPorImovelId(identificadorImovel) {
    ...
    const reservas = Array.isArray(state.clienteVisualizacaoCarteira?.reservasAtivas)
      ...
      : [];
    return reservas.find((item) => normalizarIdentificador(item?.imovel_id || item?.imovel?.id) === alvo) || null;
  }
  ...
  function renderViewReservations(reservas = []) {
    if (!Array.isArray(reservas) || !reservas.length) {
      return renderViewEmpty("Este cliente ainda nao possui simulacoes reservadas de imoveis.");
    }
      ...
      <div class="tl-clientes-view-reservas">
        ${reservas.map((reserva) => {
          ...
          const metricas = [
            ...
            ["Saldo pos-entrega", moedaVisual(negociacao.saldo_pos_entrega)],
          ].filter(([, valor]) => valor !== "--");
          ...
          const chips = [
            ...
            reserva?.reservado_em ? `Criada em ${formatarDataHoraCurta(reserva.reservado_em)}` : "",
            reserva?.simulacao_id ? "Simulacao salva" : "Sem simulacao salva",
          ];
          return `
            <article class="tl-clientes-view-reserva">
              <div class="tl-clientes-view-reserva__head">
                <div>
                  ...
                  <h4>${escapeHtml(textoVisual(reserva?.imovel?.titulo, "Imovel reservado"))}</h4>
                  <p>${escapeHtml(textoVisual(local, "Localizacao nao informada"))}</p>
                </div>
                <button class="tl-imoveis-btn tl-imoveis-btn--secondary" type="button" data-action="view-abrir-reserva" data-id="${escapeHtml(reserva.imovel_id)}">Continuar no simulador</button>
              </div>
              <div class="tl-clientes-view-chips">${chips.map(renderViewChip).join("")}</div>
              ...
              ${reserva?.observacoes ? `<small class="tl-clientes-view-reserva__note">${escapeHtml(reserva.observacoes)}</small>` : ""}
            </article>`;
        }).join("")}
      </div>`;
  }
            ...
            ${documentos.length
              ? documentos.map((documento) => `
                  <div class="tl-clientes-view-doc-item">
                    ...
                    <small>${escapeHtml([extensaoArquivo(documento?.nome_arquivo) || "ARQ", tamanhoArquivoLegivel(documento?.tamanho_bytes) || "Tamanho nao informado", formatarDataHoraCurta(documento?.data_hora_criacao) || "Data nao informada"].join(" • "))}</small>
  ...
  function renderVisualizacaoCliente(cliente, composicao = [], carteira = null) {
    ...
    const responsavel = cliente?.usuario_cadastro_nome || cliente?.usuario_cadastro_email || "Responsavel nao registrado";
    ...
    el.viewContent.innerHTML = `
      <header class="tl-clientes-view-hero">
        ...
        <div class="tl-clientes-view-hero__actions">
          <span class="tl-clientes-view-status">${escapeHtml(textoVisual(cliente?.status_documental, "Status nao informado"))}</span>
      ...
      <section class="tl-clientes-view-metrics" aria-label="Resumo do cliente">
        ${renderViewMetric("Renda total", moedaVisual(rendaTotal), "base + composicao")}
      ...
      <section class="tl-clientes-view-panel tl-clientes-view-panel--identity tl-clientes-view-panel--wide">
        ...
        <div class="tl-clientes-view-profile-grid">
          ...
          ${renderViewProfileCard("Endereco", [
      ...
      <section class="tl-clientes-view-panel tl-clientes-view-panel--finance tl-clientes-view-panel--wide">
        <div class="tl-clientes-view-panel__head">
          <span class="tl-imoveis-eyebrow mono-font">RENDA E CREDITO</span>
        ...
        <div class="tl-clientes-view-info-grid tl-clientes-view-info-grid--simulation tl-clientes-view-info-grid--finance-wide">
          ...
          ${renderViewInfo("Composicao / complementar", moedaVisual(rendaComplementar), { className: "tl-clientes-view-info--amount" })}
          ...
          ${renderViewInfo("Cartao", moedaVisual(cliente?.cartao_credito), { className: "tl-clientes-view-info--amount" })}
          ...
          ${renderViewInfo("Despesas variaveis", moedaVisual(cliente?.despesas_variaveis), { className: "tl-clientes-view-info--amount" })}
          ${renderViewInfo("Dependentes", textoVisual(cliente?.dependentes, "0"), { detalhe: `${textoVisual(cliente?.filhos, "0")} filho(s)` })}
          ${renderViewInfo("Moradores", textoVisual(cliente?.moradores, "0"), { detalhe: cliente?.veiculo === null || cliente?.veiculo === undefined ? "Veiculo nao informado" : `Veiculo: ${boolVisual(cliente?.veiculo)}` })}
          ${renderViewInfo("Imovel proprio", boolVisual(cliente?.imovel_proprio), { detalhe: cliente?.financiamentos || "Sem financiamento informado" })}
        </div>
      </section>
      ...
      <section class="tl-clientes-view-panel tl-clientes-view-panel--wide">
        <div class="tl-clientes-view-panel__head">
          <span class="tl-imoveis-eyebrow mono-font">PARAMETROS DA SIMULACAO</span>
          <strong>Preenchimento automatico no simulador</strong>
        </div>
        <div class="tl-clientes-view-info-grid tl-clientes-view-info-grid--simulation">
          ...
          ${renderViewInfo("Subsidio", moedaVisual(parametros.subsidio))}
          ${renderViewInfo("Cheque moradia", moedaVisual(parametros.cheque_moradia))}
          ${renderViewInfo("Intermediaria", `${moedaVisual(parametros.parcela_intermediaria_valor)} • ${textoVisual(parametros.parcelas_intermediarias_quantidade, "0")}x`)}
          ...
          ${renderViewInfo("Reforco", `${moedaVisual(parametros.parcela_reforco_valor)} • ${textoVisual(parametros.parcelas_reforco_quantidade, "0")}x`)}
        </div>
      ...
      <section class="tl-clientes-view-grid tl-clientes-view-grid--lower">
        <article class="tl-clientes-view-panel">
          <div class="tl-clientes-view-panel__head">
            <span class="tl-imoveis-eyebrow mono-font">COMPOSICAO FAMILIAR</span>
            <strong>${escapeHtml(String(composicao.length))} vinculo(s)</strong>
          </div>
        ...
        <article class="tl-clientes-view-panel">
          <div class="tl-clientes-view-panel__head">
            <span class="tl-imoveis-eyebrow mono-font">SIMULACAO RESERVADA</span>
            <strong>${escapeHtml(String(reservas.length))} simulacao(oes)</strong>
          </div>
      ...
      ${(cliente?.documentacao_pendente || cliente?.observacoes) ? `
        <section class="tl-clientes-view-panel tl-clientes-view-panel--wide">
          <div class="tl-clientes-view-panel__head">
            <span class="tl-imoveis-eyebrow mono-font">ANOTACOES</span>
            <strong>Leitura interna</strong>
          </div>
          ${cliente?.documentacao_pendente ? `<p class="tl-clientes-view-note"><strong>Documentacao pendente:</strong> ${escapeHtml(cliente.documentacao_pendente)}</p>` : ""}
  ...
  async function abrirVisualizacaoCliente(id) {
    ...
    try {
      ...
      el.viewContent.innerHTML = `<div class="tl-clientes-view-empty">${escapeHtml(mensagemErro(erro, "Nao foi possivel carregar a visualizacao do cliente."))}</div>`;
      mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel carregar a visualizacao do cliente."));
    }
  }
  ...
  async function carregarCarteiraComercial() {
    ...
    try {
      ...
      renderCarteiraComercial();
      mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel carregar as simulacoes reservadas do cliente."));
    }
  }
  ...
  function renderLista() {
    ...
    filtrados.forEach((c) => {
      ...
      const cadastradoPor = escapeHtml(c.usuario_cadastro_nome || c.usuario_cadastro_email || "Responsavel nao registrado");
  ...
  async function salvar() {
    ...
    try {
      ...
      try {
        ...
        const textoUpload = mensagemErro(erroUpload, "Nao foi possivel enviar a foto ou os documentos.");
      ...
      if (el.btnSalvar) el.btnSalvar.textContent = "Atualizar cadastro";
      mostrarMensagem("success", eraEdicao ? "Cliente atualizado com sucesso." : "Cliente cadastrado com sucesso.");
      mostrarFormulario();
    } catch (e) {
      const textoErro = mensagemErro(e, "Nao foi possivel salvar o cliente.");
  ...
  function bindAcoes() {
    ...
    el.btnEnviarFoto?.addEventListener("click", async () => {
      try {
        ...
        mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel enviar a foto do cliente."));
      } finally {
        renderSecaoMidiasCliente();
      }
    });
    ...
    el.btnRemoverFoto?.addEventListener("click", async () => {
      if (state.pendingFotoArquivo) {
        definirFotoPendente(null);
        mostrarMensagem("info", "Selecao de foto removida.");
        return;
      }
      ...
      try {
        ...
        mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel remover a foto do cliente."));
      } finally {
        renderSecaoMidiasCliente();
      }
    });
    ...
    el.btnEnviarDocumentos?.addEventListener("click", async () => {
      try {
        ...
        mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel enviar os documentos do cliente."));
      } finally {
        renderSecaoMidiasCliente();
      }
    });
    ...
    el.documentosLista?.addEventListener("click", async (event) => {
      ...
      try {
        ...
        mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel remover o documento do cliente."));
      } finally {
        renderSecaoMidiasCliente();
      }
    });
    ...
    el.viewModal?.addEventListener("click", (event) => {
      ...
      if (acao === "view-editar" && id) {
        ...
        void abrirEdicaoCliente(id).catch((erro) => {
          mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel carregar o cliente."));
        });
        return;
      }
    ...
    el.carteiraComercialLista?.addEventListener("click", (event) => {
      ...
      if (acao === "cancelar-reserva-cliente") {
        void cancelarReservaCarteira(id).catch((erro) => {
          mostrarMensagem("error", mensagemErro(erro, "Nao foi possivel cancelar a reserva deste cliente."));
        });
      }
    });
