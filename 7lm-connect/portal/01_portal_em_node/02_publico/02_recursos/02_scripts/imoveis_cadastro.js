(function () {
  ...
  function renderEvolucaoObra(itens, percentualAtual = 0) {
    ...
    if (!historico.length) {
      el.evolucaoLista.innerHTML = `
        <div class="tl-imoveis-empty">
          ...
          <p>Ao salvar novos marcos da obra, eles aparecerao aqui em ordem de data.</p>
        </div>
      `;
      return;
    }
    ...
    el.evolucaoLista.innerHTML = historico.map((item, index) => {
      ...
      const data = shared.formatDate(item.data_referencia || item.data_hora_criacao);
    ...
    if (!files.length) {
      el.arquivosSelecionados.innerHTML = `
        <div class="tl-imoveis-empty">
          ...
          <p>Voce pode adicionar varias fotos e videos, inclusive em selecoes separadas, antes de salvar o imovel.</p>
        </div>
      `;
      return;
    }
  ...
  function montarPayloadEvolucaoObra() {
    ...
    if (!Number.isFinite(percentual) || percentual < 0 || percentual > 100) {
      shared.showInlineMessage(el.feedback, "error", "Informe um percentual de evolucao entre 0 e 100.");
      return null;
    }

    return {
      percentual_conclusao_obra: percentual,
      data_referencia: el.evolucaoData?.value || todayInputValue(),
  ...
  async function registrarEvolucaoObra(event) {
    ...
    try {
      ...
      shared.showInlineMessage(el.feedback, "success", resposta.mensagem || "Evolucao da obra registrada com sucesso.");
    } catch (error) {
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel registrar a evolucao da obra.");
    } finally {
  ...
  async function registrarEvolucaoObraLoteEndereco() {
    ...
    const confirmado = window.confirm(
      "Aplicar esta evolucao em todos os imoveis com o mesmo endereco deste cadastro? O registro individual continuara disponivel."
    );
    ...
    try {
      ...
      if (el.evolucaoPercentual) el.evolucaoPercentual.value = "";
      await carregarImovel();
      shared.showInlineMessage(el.feedback, "success", resposta.mensagem || "Evolucao aplicada aos imoveis do mesmo endereco.");
    } catch (error) {
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel aplicar a evolucao em lote.");
    } finally {
      alternarBotoesEvolucao(false);
    }
  }
