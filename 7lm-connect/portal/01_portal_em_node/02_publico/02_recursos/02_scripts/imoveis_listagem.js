(function () {
  ...
  const el = {
    ...
    inputBairro: document.getElementById("imoveisBairro"),
  ...
  const statusMap = { disponivel: "Disponivel", reservado: "Reservado", vendido: "Vendido", inativo: "Inativo" };
  ...
  const local = (item) => [item?.bairro, item?.cidade].map(text).filter(Boolean).join(" - ") || "Local nao informado";
  ...
  const itemLabel = (item) => {
    const title = text(item?.titulo) || "Imovel sem titulo";
    const parts = [title];
    const place = local(item);
    if (place && place !== "Local nao informado") parts.push(place);
  ...
  const reservationClientName = (item) => text(reservation(item)?.cliente?.nome_completo) || "cliente nao informado";
  ...
  const reservationDetails = (item) => {
    ...
    if (!reserva) {
      return sKey(item?.status) === "reservado"
        ? "Unidade marcada como reservada sem negociacao vinculada."
        : "";
    }
    const partes = [];
    if (reserva.reservado_em) partes.push(`Reserva em ${dateLabel(reserva.reservado_em)}`);
    if (money(reserva.negociacao?.entrada)) partes.push(`Entrada ${money(reserva.negociacao.entrada)}`);
    if (money(reserva.negociacao?.valor_total_operacao)) partes.push(`Operacao ${money(reserva.negociacao.valor_total_operacao)}`);
    return partes.join(" • ");
  };
  const coord = (v) => {
  ...
  function setMapFrameEmbed(url, title) {
    ...
    el.mapaFrame.title = title || "Mapa do imovel";
  }
  ...
  function queryAtual() {
    return {
      ...
      bairro: text(el.inputBairro?.value),
  ...
  function describeExportFilters() {
    ...
    if (text(atual.bairro)) filtros.push(`Bairro: ${text(atual.bairro)}`);
  ...
  function describeExportLocation(item) {
    return [text(item?.bairro), text(item?.cidade), text(item?.estado)].filter(Boolean).join(" - ") || "Local nao informado";
  }
  ...
  function buildPortfolioAlerts(metricas) {
    ...
    if (metricas.semMidias > 0) {
      alertas.push(`${formatIntegerReport(metricas.semMidias)} unidade(s) ainda nao possuem midias publicadas.`);
    }
    if (metricas.inativos > 0) {
      alertas.push(`${formatIntegerReport(metricas.inativos)} unidade(s) estao marcadas como inativas e precisam de revisao.`);
    }
    if (metricas.disponiveis > 0) {
      alertas.push(`${formatIntegerReport(metricas.disponiveis)} unidade(s) estao livres para simulacao e negociacao.`);
    }
    if (!alertas.length) {
      alertas.push("Nenhum alerta critico foi identificado para os filtros desta exportacao.");
    }
    return alertas;
  }
  ...
  function renderPortfolioRows(items) {
    if (!items.length) {
      return '<tr><td colspan="8">Nenhum imovel encontrado para os filtros informados.</td></tr>';
    }
    ...
    return items.map((item, index) => {
      ...
      return `
        <tr>
          <td>
            <strong>${esc(`${index + 1}. ${text(item?.titulo) || "Imovel nao informado"}`)}</strong>
            <div class="table-muted">${esc(describeExportDetails(item))}</div>
          </td>
          <td>
            ${esc(describeExportLocation(item))}
            <div class="table-muted">${esc(address(item) || "Endereco nao informado")}</div>
          </td>
          <td>${esc(status)}</td>
  ...
  function montarHtmlRelatorioCarteiraImoveis(items) {
    ...
    const distribuicao = `Disponiveis ${formatIntegerReport(metricas.disponiveis)} | Reservados ${formatIntegerReport(metricas.reservados)} | Vendidos ${formatIntegerReport(metricas.vendidos)} | Inativos ${formatIntegerReport(metricas.inativos)} | Cidades ${formatIntegerReport(metricas.cidades.size)}`;
    ...
    const unidadeDestaqueCampos = [
      ["Unidade em destaque", text(destaque?.titulo) || "Nenhum imovel encontrado"],
      ["Empreendimento", text(destaque?.empreendimento) || "Nao informado"],
      ["Localizacao", localDestaque],
      ["Endereco", address(destaque) || "Nao informado"],
      ["Valor do imovel", formatMoneyReport(destaque?.valor)],
      ["Area", formatAreaReport(destaque?.area_m2)],
      ...
      ["Pre-obra minima", formatMoneyReport(destaque?.valor_parcela_minima_pre_obra)],
      ["Data de entrega", formatDateReport(destaque?.data_entrega)],
      ["Midias publicadas", formatIntegerReport(destaque?.quantidade_midias)],
    ];
    const resumoFinanceiroCampos = [
      ...
      ["Ticket medio", formatMoneyReport(metricas.ticketMedio)],
      ["Garantido medio", formatMoneyReport(metricas.valorGarantidoMedio)],
      ["Pre-obra media", formatMoneyReport(metricas.preObraMedia)],
      ["Conclusao media", formatPercentReport(metricas.conclusaoMedia)],
      ["Midias totais", formatIntegerReport(metricas.totalMidias)],
      ["Sem midias", formatIntegerReport(metricas.semMidias)],
      ["Reservas ativas", formatIntegerReport(metricas.reservasAtivas)],
      ["Proxima entrega", metricas.proximaEntrega ? formatDateReport(metricas.proximaEntrega) : "-"],
    ];
...
<head>
  ...
  <title>7LM - Relatorio da carteira de imoveis</title>
  <style>
...
<body>
  ...
  <main class="page">
    <section class="hero">
      ...
      <h1>Relatorio da carteira de imoveis</h1>
      ...
      <div class="hero-grid">
        <div class="photo">${fotoUrl ? `<img src="${esc(fotoUrl)}" alt="${esc(text(destaque?.titulo) || "Imovel")}" />` : '<div class="photo-empty">Sem foto principal</div>'}</div>
        <div class="summary">
          ...
          <article><span>Ticket medio</span><strong>${esc(formatMoneyReport(metricas.ticketMedio))}</strong></article>
          <article><span>Reservas ativas</span><strong>${esc(formatIntegerReport(metricas.reservasAtivas))}</strong></article>
          <article><span>Proxima entrega</span><strong>${esc(metricas.proximaEntrega ? formatDateReport(metricas.proximaEntrega) : "-")}</strong></article>
        </div>
      </div>
    </section>
    ...
    <p class="footer">Documento gerado pelo 7LM Connect. Valores sujeitos a validacao comercial e documental.</p>
  </main>
  <script>
  ...
  async function exportarImoveis(formato, botao) {
    ...
    try {
      if (reportWindow) {
        ...
        reportWindow.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Preparando relatorio</title><style>body{margin:0;display:grid;place-items:center;min-height:100vh;font-family:Inter,Arial,sans-serif;background:#eef8fb;color:#172033}article{padding:24px 28px;border-radius:22px;background:#fff;box-shadow:0 18px 40px rgba(18,24,38,.12)}strong{display:block;margin-bottom:8px;font-size:18px}</style></head><body><article><strong>Preparando relatorio da carteira...</strong><span>Carregando os imoveis filtrados e montando o PDF no padrao do simulador.</span></article></body></html>`);
      ...
      if (format === "pdf") {
        ...
        if (reportWindow) {
          reportWindow.document.open();
          reportWindow.document.write(montarHtmlRelatorioCarteiraImoveis(itens));
          reportWindow.document.close();
          reportWindow.focus();
        }
        shared.showInlineMessage(el.feedback, "success", "Relatorio aberto. Use a opcao Salvar como PDF na janela de impressao.");
        return;
      }
      ...
      shared.showInlineMessage(el.feedback, "success", `Exportacao ${labels[format]} pronta. O download foi iniciado.`);
    } catch (error) {
      if (reportWindow) {
        reportWindow.close();
      }
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel exportar os imoveis.");
    } finally {
      if (botao instanceof HTMLButtonElement) botao.disabled = false;
      if (el.btnAbrirExportacao) el.btnAbrirExportacao.disabled = false;
      exportButtons.forEach((item) => {
  ...
  function syncPermissions() {
    permissions.canCreate = shared.hasPermission(state.user, "imoveis.create");
    permissions.canEdit = shared.hasPermission(state.user, "imoveis.edit");
    permissions.canDelete = shared.hasPermission(state.user, "imoveis.delete");
    permissions.canMedia = shared.hasPermission(state.user, "imoveis.media.manage");
  ...
  function groupItems(items) {
    ...
    items.forEach((item) => {
      ...
      const andarNome = text(a.andar || a.pavimento || "Andar nao informado");
  ...
  function renderAcoes(id) {
    const btns = [`<button class="tl-imoveis-btn tl-imoveis-btn--secondary" data-action="ver-imovel" data-id="${esc(id)}">Ver imovel</button>`];
    if (canOpenEditor()) btns.push(`<button class="tl-imoveis-btn" data-action="editar" data-id="${esc(id)}">${permissions.canEdit ? "Editar" : "Gerenciar midias"}</button>`);
    ...
    return `
      <article class="tl-imoveis-unit">
        <div class="tl-imoveis-unit__head">
          <div>
            <span class="tl-imoveis-eyebrow mono-font">${esc(item.tipo_imovel || "Imovel")}</span>
            <div class="tl-imoveis-unit__title">${esc(item.titulo || "Imovel")}</div>
          </div>
        ...
        <div class="tl-imoveis-unit__stats">
          ...
          <span class="tl-imoveis-unit__stat">Area: ${esc(area(item.area_m2))}</span>
          <span class="tl-imoveis-unit__stat">Midias: ${esc(String(item.quantidade_midias || 0))}</span>
        </div>
    ...
    return `
      <details class="tl-imoveis-bloco">
        <summary class="tl-imoveis-bloco__summary">
          ...
          <div class="tl-imoveis-group__chips"><span class="tl-imoveis-chip">${esc(String(bloco.total))} imovel(is)</span>${statusChips(statusCounts(bloco.itens))}</div>
    ...
    return `
      <details class="tl-imoveis-local">
        <summary class="tl-imoveis-local__summary">
          ...
          <div class="tl-imoveis-local__chips"><span class="tl-imoveis-chip">${esc(String(locObj.total))} imovel(is)</span><span class="tl-imoveis-chip">${esc(String(locObj.blocos.length))} bloco(s)</span><span class="tl-imoveis-chip">${esc(String(locObj.totalAndares))} andar(es)</span>${statusChips(statusCounts(locObj.itens))}</div>
        </summary>
  ...
  function renderCard(item, stats) {
    ...
    const reservaMeta = reservationDetails(item);
    return `
      <article class="tl-imoveis-showcase-card" data-id="${id}" tabindex="0" role="button" aria-label="Abrir showroom de ${esc(item.titulo || "Imovel")}">
        <div class="tl-imoveis-showcase-card__media">
          ${foto ? `<img src="${esc(foto)}" alt="${esc(item.titulo || "Imovel")}" loading="lazy" class="tl-imoveis-showcase-card__hero" />` : `<div class="tl-imoveis-showcase-card__hero tl-imoveis-showcase-card__hero--empty"><span>Sem foto principal</span></div>`}
        ...
        <div class="tl-imoveis-showcase-card__body">
          <div class="tl-imoveis-showcase-card__top">
            <span class="tl-imoveis-eyebrow mono-font">${esc(item.tipo_imovel || "Imovel")}</span>
            <h3>${esc(item.titulo || "Imovel")}</h3>
            <p>${esc(item?.detalhes_comerciais?.empreendimento || item?.agrupamento?.localidade || "Empreendimento nao informado")}</p>
          ...
          <div class="tl-imoveis-showcase-card__meta">
            ...
            <span><b>${esc(orientacao(item))}</b><small>Orientacao</small></span>
            <span><b>${esc(String(num(item.vagas_garagem) ?? "-"))}</b><small>Vagas</small></span>
            <span><b>${esc(area(item.area_m2))}</b><small>Area</small></span>
  ...
  function renderList(items) {
    if (!items.length) {
      el.lista.innerHTML = `<article class="tl-imoveis-empty"><strong>Nenhum imovel encontrado</strong><p>Refine os filtros para continuar.</p></article>`;
      renderCompareDock();
    ...
    if (state.viewMode === "tecnica") {
      ...
      renderCompareDock();
      return `${items.length} imovel(is) em ${locs.length} localidade(s).`;
    }
    ...
    return `${items.length} imovel(is). Disponiveis: ${c.disponivel}, Reservados: ${c.reservado}, Vendidos: ${c.vendido}, Inativos: ${c.inativo}.`;
  }

  function renderLoading() {
    el.lista.innerHTML = `<article class="tl-imoveis-empty"><strong>Carregando imoveis...</strong><p>Aguarde enquanto consultamos a base.</p></article>`;
    if (el.resumo) el.resumo.textContent = "Carregando listagem de imoveis...";
  }

  function updatePagination(meta) {
    state.paginacao = meta;
    shared.toggleHidden(el.paginationWrap, meta.total_paginas <= 1);
    if (el.paginaInfo) el.paginaInfo.textContent = `Pagina ${meta.pagina} de ${meta.total_paginas} - ${meta.total} registro(s)`;
  ...
  async function carregarImoveis() {
    ...
    renderLoading();
    try {
      const payload = await shared.apiRequest(`${ENDPOINT_LISTAR}?${queryString(queryAtual())}`, { method: "GET" });
      ...
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel carregar os imoveis.");
      ...
      if (el.resumo) el.resumo.textContent = "Nao foi possivel carregar os imoveis.";
    }
  }
  ...
  async function importarPlanilha(event) {
    ...
    try {
      ...
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel importar a planilha.");
    } finally {
  ...
  function renderEvolucaoLoteOptions(selectedId = "") {
    ...
    if (!items.length) {
      el.evolucaoReferencia.innerHTML = `<option value="">Nenhum imovel carregado na tela</option>`;
  ...
  function renderEvolucaoLoteList(matches, item) {
    ...
    if (!matches.length) {
      el.evolucaoLista.innerHTML = `
        <article class="tl-imoveis-evolucao-lote__empty">
          <strong>Nenhuma unidade disponivel para selecao</strong>
          <p>Escolha uma unidade de referencia com endereco base valido para carregar o lote desta tela.</p>
        </article>`;
      if (el.evolucaoSelecaoResumo) el.evolucaoSelecaoResumo.textContent = "Nenhuma unidade selecionada";
      if (el.evolucaoSelecaoHint) el.evolucaoSelecaoHint.textContent = "Assim que o lote for identificado, voce podera marcar varias unidades e aplicar a mesma evolucao.";
    ...
    if (el.evolucaoSelecaoHint) {
      const referencia = text(item?.titulo) || "Unidade de referencia";
      el.evolucaoSelecaoHint.textContent = `Lote carregado a partir de ${referencia}. Clique nas unidades abaixo para incluir ou remover da atualizacao.`;
    }
    ...
    el.evolucaoLista.innerHTML = matches.map((candidate) => {
      ...
      const meta = [
        ...
        local(candidate) !== "Local nao informado" ? local(candidate) : "",
      ].filter(Boolean).join(" • ");
      return `
        <button
        ...
        >
          ...
          <span class="tl-imoveis-evolucao-lote__copy">
            <strong>${esc(text(candidate?.titulo) || "Imovel sem titulo")}</strong>
            <span>${esc(meta || address(candidate) || "Endereco nao informado")}</span>
          </span>
  ...
  function refreshEvolucaoLoteSummary({ resetSelection = false } = {}) {
    ...
    if (!item) {
      ...
      if (el.evolucaoResumoEndereco) el.evolucaoResumoEndereco.textContent = "O sistema vai procurar todas as unidades com o mesmo endereco base da referencia escolhida.";
      if (el.evolucaoResumoContagem) el.evolucaoResumoContagem.textContent = "Escolha uma unidade de referencia para carregar o lote.";
    ...
    if (el.evolucaoResumoLote) {
      el.evolucaoResumoLote.textContent = reference.base
        ? `${visibleCount} unidade(s) com o mesmo endereco nesta tela`
        : "Endereco base insuficiente para atualizar em lote";
    }
    if (el.evolucaoResumoEndereco) {
      el.evolucaoResumoEndereco.textContent = address(item) || "Endereco nao informado para a unidade selecionada.";
    }
    if (el.evolucaoResumoContagem) {
      if (!reference.base) {
        el.evolucaoResumoContagem.textContent = "A unidade escolhida precisa ter endereco base suficiente para aplicar a evolucao em lote.";
      } else {
        const selectedCount = state.evolucaoSelecionados.size;
        el.evolucaoResumoContagem.textContent = `Referencia: ${text(item.titulo) || "Imovel"} • ${selectedCount} de ${visibleCount} unidade(s) marcadas para esta atualizacao.`;
      }
    }
  ...
  function openEvolucaoLoteModal(preferredId = "") {
    ...
    if (!Array.isArray(state.items) || !state.items.length) {
      shared.showInlineMessage(el.feedback, "warning", "Carregue a listagem de imoveis antes de atualizar a obra em lote.");
      return;
    }
  ...
  async function aplicarEvolucaoLote(event) {
    ...
    if (!identificadorImovel || !item) {
      shared.showInlineMessage(el.feedback, "error", "Selecione uma unidade de referencia para aplicar a evolucao em lote.");
      return;
    }
    if (!identificadoresSelecionados.length) {
      shared.showInlineMessage(el.feedback, "error", "Selecione pelo menos uma unidade do lote para aplicar a evolucao da obra.");
      return;
    }
    if (percentual === null || percentual < 0 || percentual > 100) {
      shared.showInlineMessage(el.feedback, "error", "Informe um percentual da obra valido entre 0 e 100.");
      el.evolucaoPercentual?.focus();
      return;
    }
    if (!lotAddressData(item).base) {
      shared.showInlineMessage(el.feedback, "error", "A unidade escolhida nao possui endereco suficiente para atualizar o lote.");
      return;
    }

    const payload = {
      percentual_conclusao_obra: Number(percentual.toFixed(2)),
      data_referencia: text(el.evolucaoData?.value) || dateInputValue(),
    ...
    try {
      ...
      shared.showInlineMessage(el.feedback, "success", response?.mensagem || "Evolucao da obra aplicada em lote com sucesso.");
      await carregarImoveis();
    } catch (error) {
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel aplicar a evolucao da obra em lote.");
    } finally {
  ...
  function openLightbox(midias, idx) {
    ...
    lb.innerHTML = `
      ...
      <div class="tl-imoveis-lightbox__content">
        <div class="tl-imoveis-lightbox__panel" role="dialog" aria-modal="true" aria-label="Visualizador de imagens do imovel">
          <div class="tl-imoveis-lightbox__toolbar">
            <div class="tl-imoveis-lightbox__headline">
              ...
              <strong class="tl-imoveis-lightbox__caption">Visualizacao ampliada</strong>
              <small class="tl-imoveis-lightbox__hint">Role para aproximar, de duplo clique para destacar e arraste quando estiver em zoom.</small>
            </div>
          ...
          <div class="tl-imoveis-lightbox__stage">
            ...
            <div class="tl-imoveis-lightbox__image-wrap">
              <img class="tl-imoveis-lightbox__img" alt="Midia" />
            </div>
            <button class="tl-imoveis-lightbox__nav tl-imoveis-lightbox__next" type="button" aria-label="Proxima imagem">&rsaquo;</button>
          </div>
    ...
    const show = (x) => {
      ...
      if (img) {
        ...
        img.src = src;
        img.alt = foto.nome_arquivo || "Midia";
        img.onload = () => img.classList.remove("is-loading");
      }
      if (caption) caption.textContent = foto.nome_arquivo || state.showroomItem?.titulo || "Visualizacao ampliada";
  ...
  function renderCarousel() {
    ...
    if (!arr.length) {
      stage.innerHTML = `<div class="tl-imoveis-showroom__media-empty"><strong>Sem fotos disponiveis</strong><p>Este imovel ainda nao possui imagens cadastradas.</p></div>`;
      thumbs.innerHTML = "";
    ...
    stage.innerHTML = `
      ...
      <div class="tl-imoveis-showroom__stage-inner" data-showroom-stage="1">
        ${m.tipo_arquivo === "video" ? `<video src="${esc(m.caminho_arquivo)}" controls preload="metadata" class="tl-imoveis-showroom__media-video"></video>` : `<img src="${esc(m.caminho_arquivo)}" alt="${esc(m.nome_arquivo || state.showroomItem?.titulo || "Midia")}" class="tl-imoveis-showroom__media-img" loading="eager" />`}
    ...
    thumbs.innerHTML = arr.map((x, i) => `<button class="tl-imoveis-showroom__thumb ${i === state.showroomIndex ? "is-active" : ""}" type="button" data-action="show-go" data-index="${i}">${x.tipo_arquivo === "video" ? "<span class='tl-imoveis-showroom__thumb-video'>VIDEO</span>" : `<img src="${esc(x.caminho_arquivo)}" alt="${esc(x.nome_arquivo || "Miniatura")}" loading="lazy" />`}</button>`).join("");
  ...
  function infoShowroom(item) {
    ...
    const difs = Array.isArray(item?.diferenciais_comerciais)
      ? item.diferenciais_comerciais
          ...
          .slice(0, 10)
      : [];
    const empreendimento = d.empreendimento || item?.agrupamento?.localidade || item?.bairro || "Empreendimento nao informado";
    const localCompacto = [locx.bairro || item?.bairro, locx.cidade || item?.cidade, locx.estado || item?.estado].filter(Boolean).join(" - ") || "Bairro e cidade nao informados";
    const enderecoCompleto = address(item) || "Endereco nao informado";
    ...
    const disponibilidadeLabel = statusLabel === "Disponivel" ? "Pronto para proposta" : statusLabel;
    const reservaAtual = reservation(item);
    const reservaLabel = reservationLabel(item);
    const reservaMeta = reservationDetails(item);
    const showroomSimularLabel = reservaAtual?.simulacao_id
      ? "Continuar negociacao"
      : (reservaAtual ? "Abrir reserva no simulador" : "Simular com este imovel");
    const vistaInfo = text(d.vista) || extrairValorDiferencial(item, "Vista");
    const condicaoInfo = text(d.condicao_especial) || extrairValorDiferencial(item, "Condicao especial");
    ...
    const infoCards = [
      ...
      { label: "Orientacao", value: orientacao(item) },
      { label: "Vagas", value: String(num(item.vagas_garagem) ?? "-") },
      { label: "Area privativa", value: areaPrivativa },
      { label: "Area total", value: areaTotal },
      { label: "Data entrega", value: dateLabel(item.data_entrega) || "-" },
      { label: "Entrega", value: `${num(item.meses_pre_entrega) ?? 36} meses` },
      { label: "Pos-entrega", value: `${num(item.meses_pos_entrega) ?? 24} meses` },
      { label: "Obra", value: percent(item.percentual_conclusao_obra || 0) },
      { label: "Posicao", value: text(d.posicao) || "-" },
      possuiSuite ? { label: "Suite", value: "Sim" } : null,
      vistaInfo ? { label: "Vista", value: vistaInfo } : null,
      condicaoInfo ? { label: "Condicao", value: condicaoInfo } : null,
    ].filter(Boolean);
    return {
      left: `
      <div class="tl-imoveis-showroom__actions-wrap">
        <div class="tl-imoveis-showroom__actions-head">
          <span class="tl-imoveis-eyebrow mono-font">ACOES</span>
          <small>Atalhos da unidade</small>
        </div>
        <div class="tl-imoveis-showroom__actions tl-imoveis-showroom__actions--primary">
          <button class="tl-imoveis-btn tl-imoveis-showroom__action-btn--wide" type="button" data-action="show-simular">${esc(showroomSimularLabel)}</button>
          <button class="tl-imoveis-btn tl-imoveis-btn--secondary" type="button" data-action="show-reservar" ${permissions.canEdit ? "" : "disabled"}>Reservar</button>
          <button class="tl-imoveis-btn tl-imoveis-btn--secondary" type="button" data-action="show-vender" ${permissions.canEdit ? "" : "disabled"}>Marcar como venda</button>
        </div>
        <div class="tl-imoveis-showroom__actions tl-imoveis-showroom__actions--secondary">
          ...
          <button class="tl-imoveis-btn tl-imoveis-btn--ghost" type="button" data-action="show-copy">Copiar localizacao</button>
      ...
      <div class="tl-imoveis-showroom__hero-card">
        ...
        <div class="tl-imoveis-showroom__quick-strip">
          <article><span>Area privativa</span><strong>${esc(areaPrivativa)}</strong></article>
      ...
      <div class="tl-imoveis-showroom__reservation">
        <div class="tl-imoveis-showroom__section-head">
          ...
          <small>${esc(reservaAtual ? "Negociacao vinculada" : "Marcacao manual")}</small>
        </div>
        <strong>${esc(reservaLabel)}</strong>
        <p>${esc(reservaMeta || "Essa unidade esta reservada, mas ainda nao ha detalhes extras disponiveis.")}</p>
        ${reservaAtual ? `
          <div class="tl-imoveis-showroom__reservation-metrics">
            ...
            <article><span>FGTS / Subsidio / Cheque moradia</span><strong>${esc([money(reservaAtual.negociacao?.fgts), money(reservaAtual.negociacao?.subsidio), money(reservaAtual.negociacao?.cheque_moradia)].filter(Boolean).join(" + ") || "-")}</strong></article>
          </div>` : ""}
      </div>` : ""}
      <div class="tl-imoveis-showroom__section-block">
        <div class="tl-imoveis-showroom__section-head">
          <span class="tl-imoveis-eyebrow mono-font">INFORMACOES DA UNIDADE</span>
      ...
      <div class="tl-imoveis-showroom__footer-grid">
        <div class="tl-imoveis-showroom__location">
          <div class="tl-imoveis-showroom__location-head">
            <span class="tl-imoveis-eyebrow mono-font">LOCALIZACAO</span>
        ...
        <div class="tl-imoveis-showroom__timeline">
          <div class="tl-imoveis-showroom__section-head">
            <span class="tl-imoveis-eyebrow mono-font">RESUMO RAPIDO</span>
            <small>Atualizacao comercial</small>
          </div>
          <ul>
            ...
            <li><span>Atualizacao</span><strong>${esc(shared.formatDate(item.data_hora_atualizado_em) || "-")}</strong></li>
          </ul>
        </div>
      </div>`,
    };
  ...
  async function openShowroom(id) {
    ...
    if (el.showroomContent) el.showroomContent.innerHTML = `<article class="tl-imoveis-empty"><strong>Carregando showroom...</strong><p>Estamos consultando as midias e detalhes comerciais.</p></article>`;
    try {
      const item = await detalhe(id, false);
      if (!item) throw new Error("Imovel nao encontrado para showroom.");
      renderShowroom(item);
    } catch (error) {
      if (el.showroomContent) el.showroomContent.innerHTML = `<article class="tl-imoveis-empty"><strong>Falha ao abrir showroom</strong><p>${esc(error.message || "Nao foi possivel carregar o imovel.")}</p></article>`;
    }
  }
  ...
  async function openMapModal(item) {
    ...
    if (el.mapaEndereco) el.mapaEndereco.textContent = state.mapaAtual.endereco || "Endereco nao informado.";
    if (el.mapaCidade) el.mapaCidade.textContent = state.mapaAtual.cidade || "Cidade e bairro nao informados.";
    ...
    if (embedUrl) {
      ...
      setMapFrameEmbed(embedUrl, `Mapa do imovel ${item?.titulo || ""}`.trim());
      return;
    }
    setMapFrameDoc(
      "Nao foi possivel abrir o mapa aqui",
      "Os atalhos do Google Maps continuam disponiveis no card ao lado para seguir com a localizacao desta unidade.",
  ...
  async function copyText(value, success) {
    const v = text(value);
    if (!v) return shared.showInlineMessage(el.feedback, "warning", "Nao ha conteudo para copiar.");
    try {
      ...
      shared.showInlineMessage(el.feedback, "error", "Nao foi possivel copiar para a area de transferencia.");
    }
  }

  function shareSummary(item) {
    return [
      `Unidade: ${item?.titulo || "Imovel"}`,
  ...
  async function share(item) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: item.titulo || "Imovel", text: resumo, url });
    ...
    try {
      window.sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          simulacao_id: reservaAtual?.simulacao_id || "",
  ...
  function payloadStatus(item, status) {
    return {
      titulo: item.titulo || "Imovel sem titulo",
  ...
  async function updateStatus(id, status) {
    if (!permissions.canEdit) return shared.showInlineMessage(el.feedback, "warning", "Seu perfil nao possui permissao para alterar status.");
    const item = await detalhe(id, true);
    if (!item) return shared.showInlineMessage(el.feedback, "error", "Nao foi possivel carregar o imovel.");
  ...
  async function removeItem(id) {
    ...
    if (!window.confirm("Deseja realmente excluir este imovel? Esta acao remove tambem as midias associadas.")) return;
    ...
    saveFavoritos();
    renderCompareDock();
    shared.showInlineMessage(el.feedback, "success", "Imovel excluido com sucesso.");
    await carregarImoveis();
    toggleModal(el.showroomSection, false);
  }

  function openEditor(id) {
    if (!canOpenEditor()) return shared.showInlineMessage(el.feedback, "warning", "Seu perfil permite somente visualizacao.");
  ...
  function toggleCompare(id) {
    ...
    if (state.comparacao.has(key)) {
      ...
      shared.showInlineMessage(el.feedback, "info", "Imovel removido da comparacao.");
    } else {
      if (state.comparacao.size >= LIMITE_COMPARACAO) return shared.showInlineMessage(el.feedback, "warning", `Voce pode comparar no maximo ${LIMITE_COMPARACAO} imoveis.`);
      state.comparacao.add(key);
      if (state.comparacao.size >= 2 && sizeBefore < 2) {
        shared.showInlineMessage(el.feedback, "success", "Comparacao pronta. Abrindo painel comparativo.");
        void openComparacao();
      } else if (state.comparacao.size < 2) {
        shared.showInlineMessage(el.feedback, "info", "Selecione mais 1 imovel para comparar.");
      }
    }
    renderCompareDock();
  }
  ...
  function toggleFavorito(id) {
    ...
    if (state.favoritos.has(key)) {
      ...
      shared.showInlineMessage(el.feedback, "info", "Imovel removido dos favoritos.");
    } else {
      state.favoritos.add(key);
      shared.showInlineMessage(el.feedback, "success", "Imovel marcado como favorito.");
    }
    saveFavoritos();
  }

  async function openComparacao() {
    if (state.comparacao.size < 2) return shared.showInlineMessage(el.feedback, "warning", "Selecione ao menos 2 imoveis para comparar.");
    toggleModal(el.comparacaoSection, true);
    if (!el.comparacaoContent) return;
    el.comparacaoContent.innerHTML = `<article class="tl-imoveis-empty"><strong>Montando comparacao...</strong><p>Estamos carregando os detalhes das unidades.</p></article>`;
    const ids = Array.from(state.comparacao);
    ...
    if (!items.length) {
      el.comparacaoContent.innerHTML = `<article class="tl-imoveis-empty"><strong>Nao foi possivel montar a comparacao</strong><p>Tente novamente em instantes.</p></article>`;
      return;
    }
    ...
    el.comparacaoContent.innerHTML = `<div class="tl-imoveis-compare-grid">${items.map((i) => {
      ...
      const badge = num(i.valor) !== null && min !== null && num(i.valor) === min ? "Menor valor" : (num(i.area_m2) !== null && maxA !== null && num(i.area_m2) === maxA ? "Maior area" : "Comparacao");
      const dif = Array.isArray(i.diferenciais_comerciais) ? i.diferenciais_comerciais.slice(0, 3) : [];
      return `<article class="tl-imoveis-compare-card"><div class="tl-imoveis-compare-card__media">${f ? `<img src="${esc(f)}" alt="${esc(i.titulo || "Imovel")}" loading="lazy" />` : `<div class="tl-imoveis-showcase-card__hero tl-imoveis-showcase-card__hero--empty"><span>Sem foto</span></div>`}<span class="tl-imoveis-commercial-badge">${esc(badge)}</span></div><div class="tl-imoveis-compare-card__body"><h3>${esc(i.titulo || "Imovel")}</h3><p>${esc(local(i))}</p><strong>${shared.formatCurrency(i.valor)}</strong><span class="tl-imoveis-badge tl-imoveis-badge--status tl-imoveis-badge--${esc(sKey(i.status))}">${esc(sLabel(i.status))}</span><ul><li><b>Tipo:</b> ${esc(i.tipo_imovel || "-")}</li><li><b>Quartos:</b> ${esc(String(num(i.quartos) ?? "-"))}</li><li><b>Orientacao:</b> ${esc(orientacao(i))}</li><li><b>Vagas:</b> ${esc(String(num(i.vagas_garagem) ?? "-"))}</li><li><b>Area:</b> ${esc(area(i.area_m2))}</li></ul>${dif.length ? `<div class="tl-imoveis-compare-card__tags">${dif.map((d) => `<span class="tl-imoveis-chip">${esc(d)}</span>`).join("")}</div>` : ""}<div class="tl-imoveis-compare-card__actions"><button class="tl-imoveis-btn" type="button" data-action="open-showroom" data-id="${esc(i.id)}">Abrir showroom</button></div></div></article>`;
    }).join("")}</div>`;
  }
  ...
  function listAction(action, id) {
    ...
    if (action === "excluir") return void removeItem(id).catch((e) => shared.showInlineMessage(el.feedback, "error", e.message || "Nao foi possivel excluir o imovel."));
    if (action === "favoritar") {
      toggleFavorito(id);
  ...
  function showroomAction(action, data) {
    ...
    if (action === "show-copy") return void copyText(address(item), "Endereco copiado com sucesso.");
    if (action === "show-share") return void share(item);
    if (action === "show-simular") return sendToSimulator(item);
    if (action === "show-reservar") return void updateStatus(item.id, "Reservado").catch((e) => shared.showInlineMessage(el.feedback, "error", e.message || "Nao foi possivel atualizar para reservado."));
    if (action === "show-vender") return void updateStatus(item.id, "Vendido").catch((e) => shared.showInlineMessage(el.feedback, "error", e.message || "Nao foi possivel atualizar para vendido."));
    if (action === "show-editar") return openEditor(item.id);
    if (action === "show-excluir") return void removeItem(item.id).catch((e) => shared.showInlineMessage(el.feedback, "error", e.message || "Nao foi possivel excluir o imovel."));
  ...
  function bindEvents() {
    ...
    el.btnLimpar?.addEventListener("click", () => {
      ...
      if (el.inputCidade) el.inputCidade.value = "";
      if (el.inputBairro) el.inputBairro.value = "";
      if (el.inputStatus) {
    ...
    el.btnMapaCopiar?.addEventListener("click", () => void copyText(state.mapaAtual?.endereco || "", "Endereco copiado com sucesso."));
  ...
  async function init() {
    try {
      ...
      shared.showInlineMessage(el.feedback, "error", error.message || "Nao foi possivel carregar a pagina de imoveis.");
    }
  }
