...
def _mensagem_cpf_cliente_duplicado(
    ...
    *,
    ...
    if responsavel == "por voce":
        return "Este CPF ja esta cadastrado em outro cliente seu. Use o cadastro existente em vez de criar um novo."
    if responsavel:
        return f"Este CPF ja esta cadastrado {responsavel}. Outro corretor nao pode cadastrar o mesmo cliente."
    return "Este CPF ja esta cadastrado na base comercial. Outro corretor nao pode cadastrar o mesmo cliente."
...
async def _validar_cpf_cliente_disponivel(
    ...
    *,
    ...
    if membro_existente:
        raise HTTPException(
            status_code=409,
            detail="Este CPF ja esta vinculado em composicao familiar ativa e nao pode ser reutilizado no cliente principal.",
        )
    ...
    *,
    ...
    if cliente_mesmo_cpf:
        if str(cliente_mesmo_cpf["identificador_cliente"]) == str(identificador_cliente_principal):
            raise HTTPException(
                status_code=409,
                detail="O CPF do membro nao pode ser igual ao CPF do cliente principal.",
            )
        raise HTTPException(
            status_code=409,
            detail="Este CPF ja pertence a um cliente principal e nao pode ser reaproveitado.",
        )
    ...
    if membro_existente:
        raise HTTPException(
            status_code=409,
            detail="Este CPF ja esta vinculado em outra composicao familiar ativa.",
        )
...
async def _buscar_cliente_visivel(conexao, identificador_cliente: str, identificador_usuario: str):
...
async def buscar_cliente(
    ...
    async with pool.acquire() as conexao:
        ...
        cliente = await _buscar_cliente_visivel(conexao, identificador_cliente, identificador_usuario)
        if not cliente:
            raise HTTPException(status_code=404, detail="Cliente nao encontrado.")
...
async def criar_cliente(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.create",
            "Voce nao tem permissao para cadastrar clientes.",
        )
...
async def atualizar_cliente(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.edit",
            "Voce nao tem permissao para editar clientes.",
        )

        cliente_atual = await _buscar_cliente_visivel(conexao, identificador_cliente, identificador_usuario)
...
async def remover_cliente(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.delete",
            "Voce nao tem permissao para remover clientes.",
        )
...
async def upload_foto_cliente(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.edit",
            "Voce nao tem permissao para enviar foto do cliente.",
        )
...
async def remover_foto_cliente(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.edit",
            "Voce nao tem permissao para remover foto do cliente.",
        )
        ...
        if not midia:
            raise HTTPException(status_code=404, detail="Foto do cliente nao encontrada.")
...
async def upload_documentos_cliente(
    ...
    if len(arquivos) > CLIENTES_TOTAL_MAXIMO_DOCUMENTOS_POR_ENVIO:
        raise HTTPException(
            status_code=400,
            detail=f"Envie no maximo {CLIENTES_TOTAL_MAXIMO_DOCUMENTOS_POR_ENVIO} documentos por vez.",
        )
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.edit",
            "Voce nao tem permissao para enviar documentos do cliente.",
        )
...
async def remover_documento_cliente(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.edit",
            "Voce nao tem permissao para remover documentos do cliente.",
        )
        ...
        if not midia:
            raise HTTPException(status_code=404, detail="Documento do cliente nao encontrado.")
...
async def listar_composicao_familiar(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.view",
            "Voce nao tem permissao para visualizar composicao familiar.",
        )
...
async def buscar_membro_composicao(
    ...
    async with pool.acquire() as conexao:
        ...
        if not membro:
            raise HTTPException(status_code=404, detail="Membro da composicao familiar nao encontrado.")
...
async def criar_membro_composicao(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.edit",
            "Voce nao tem permissao para editar composicao familiar.",
        )
    ...
    return {
        "mensagem": "Membro da composicao familiar adicionado com sucesso.",
...
async def atualizar_membro_composicao(
    ...
    return {
        "mensagem": "Membro da composicao familiar atualizado com sucesso.",
        "item": serializar_membro(membro),
    }
...
async def atualizar_flags_composicao(
    ...
    return {
        "mensagem": "Flags da composicao familiar atualizadas com sucesso.",
