(function () {
  ...
  const LIMITE_SUGESTOES_ESCOLHA_INICIAL = 5;
  const LIMITE_SUGESTOES_CALCULO_SEM_IMOVEL = 5;
  ...
  function syncPostDeliveryFieldConstraints() {
    ...
    if (el.parceiroHint) {
      ...
      el.parceiroHint.textContent = creditur
        ...
        : "";
      if (credituGeral) {
        el.parceiroHint.textContent = "Creditú Geral: Importe O PDF Oficial Para Aplicar O Valor Liberado No Pré-Chaves; A 7LM Do Pós-Creditú É Complementar E Manual.";
      }
    }
  ...
  function renderInitialPropertyChoiceOptions(options = state.escolhaImovelInicialOpcoes) {
    ...
    el.escolhaImovelOpcoes.innerHTML = options
      .map((option, index) => {
        ...
        return `
          <article class="tl-sim-escolha-imovel-option" data-tone="${escapeHtml(option.tone)}">
            <div class="tl-sim-escolha-imovel-option__top">
              <span class="tl-sim-escolha-imovel-option__role">${escapeHtml(option.role)}</span>
              <span class="tl-sim-escolha-imovel-option__number">${escapeHtml(`Opção ${optionNumber}`)}</span>
            </div>
            <div class="tl-sim-escolha-imovel-option__media">
            ...
            <div class="tl-sim-escolha-imovel-option__content">
              <strong class="tl-sim-escolha-imovel-option__title" title="${escapeHtml(imovel.titulo || "Imóvel")}">${escapeHtml(imovel.titulo || "Imóvel")}</strong>
            ...
            <div class="tl-sim-escolha-imovel-option__meta">
              <em>${escapeHtml(entrega)}</em>
              <em>Comp. ${escapeHtml(formatPercent(resumo.percentual_comprometimento || 0))}</em>
            </div>
            <button class="tl-imoveis-btn tl-sim-escolha-imovel-option__cta" type="button" data-escolha-imovel-id="${escapeHtml(id)}">
              Selecionar Imóvel
            </button>
          </article>`;
      })
  ...
  function buildCrediturPayloadIntervals(items = collectCrediturSemestres({ normalize: true })) {
    if (isCrediturGeralSelected()) {
      return buildCredituGeralPayloadIntervals(getCredituGeralApplied(), items);
    }
    const mesesPre = getCrediturConfiguredPreMonths();
  ...
  function isCredituGeralManual7lm(result = {}) {
    return result?.parcela_7lm_manual === true || result?.parcela_7lm_manual === "true";
  }

  function isCredituGeralContext(result = {}) {
    const origem = String(result?.origem || "").trim().toLowerCase();
    const modalidade = normalizeSimulationPartner(firstFilled(
      result?.modalidade,
      result?.parceiro_simulacao,
      ""
    ));
    return modalidade === PARTNER_CREDITUR_GERAL
      || origem === "pdf_creditu"
      || origem === "simulador_creditu_geral"
      || isCredituGeralPdfResult(result);
  }

  function getCredituGeralCrediturInstallmentCandidates(result = {}) {
    const storedIntervals = Array.isArray(result?.parcelas_creditur_intervalos)
      ? result.parcelas_creditur_intervalos
      : [];
    return [
      result?.parcela_inicial,
      result?.parcela_final,
      ...storedIntervals
        .filter((item) => toNumber(item?.parcela_inicio, 1) <= CREDITUR_PRE_DEFAULT_MONTHS)
        .map((item) => item?.valor),
    ]
      .map((value) => roundMoneyNumber(parseMoney(value || 0)))
      .filter((value) => value > 0);
  }

  function isCredituGeralCrediturInstallmentValue(result = {}, value = 0) {
    const valor = roundMoneyNumber(parseMoney(value || 0));
    if (valor <= 0 || !isCredituGeralContext(result)) return false;
    return getCredituGeralCrediturInstallmentCandidates(result)
      .some((candidate) => Math.abs(valor - candidate) <= 0.01);
  }

  function isLegacyCredituGeralMixed7lmValue(result = {}, value = 0) {
    if (isCredituGeralManual7lm(result)) return false;
    const valor = roundMoneyNumber(parseMoney(value || 0));
    if (valor <= 0) return false;
    return isCredituGeralCrediturInstallmentValue(result, valor);
  }
  ...
  function getCredituGeralParcela7lm(result = {}) {
    ...
    if (isCredituGeralContext(result) && !isCredituGeralManual7lm(result)) return 0;
    if (isCredituGeralCrediturInstallmentValue(result, valorInformado)) return 0;
    if (isLegacyCredituGeralMixed7lmValue(result, valorInformado)) return 0;
    return valorInformado > 0 ? roundMoneyNumber(valorInformado) : 0;
  }

  function buildCredituGeral7lmIntervals(result = getCredituGeralApplied(), items = collectCrediturSemestres({ normalize: true })) {
    const sourceItems = Array.isArray(items) ? items : [];
    const hasPostRows = sourceItems.some((item) => normalizeCrediturPhase(item.fase) === CREDITUR_PHASE_POST);
    const postRows = normalizeCrediturIntervalCoverage(sourceItems)
      .filter((item) => normalizeCrediturPhase(item.fase) === CREDITUR_PHASE_POST)
      .map((item) => {
        const inicio = normalizeCrediturParcelNumber(item.parcela_inicio, 1, getCrediturConfiguredPostMonths());
        const fim = normalizeCrediturParcelNumber(item.parcela_fim, inicio, getCrediturConfiguredPostMonths());
        return {
          parcela_inicio: CREDITUR_PRE_DEFAULT_MONTHS + Math.min(inicio, fim),
          parcela_fim: CREDITUR_PRE_DEFAULT_MONTHS + Math.max(inicio, fim),
          valor: roundMoneyNumber(parseMoney(item.valor || 0)),
        };
      })
      .filter((item) => item.valor > 0 && !isCredituGeralCrediturInstallmentValue(result, item.valor));
    if (hasPostRows) return postRows;
    if (postRows.length) return postRows;
  ...
  function buildCredituGeralPayloadIntervals(result = getCredituGeralApplied(), items = collectCrediturSemestres({ normalize: true })) {
    ...
    if (Array.isArray(result?.parcelas_creditur_intervalos) && result.parcelas_creditur_intervalos.length) {
      const credituIntervals = result.parcelas_creditur_intervalos
        .map((item) => ({
        ...
        .map((item) => item.parcela_fim < item.parcela_inicio
          ...
          : item)
        .filter((item) => item.parcela_inicio <= CREDITUR_PRE_DEFAULT_MONTHS)
        .map((item) => ({
          ...item,
          parcela_fim: Math.min(item.parcela_fim, CREDITUR_PRE_DEFAULT_MONTHS),
        }))
        .sort((a, b) => a.parcela_inicio - b.parcela_inicio || a.parcela_fim - b.parcela_fim);
      if (credituIntervals.length) return [...credituIntervals, ...buildCredituGeral7lmIntervals(result, items)];
    }
    ...
    return [...intervalos, ...buildCredituGeral7lmIntervals(result, items)];
  }

  function sanitizeCredituGeralPdfIntervals(intervals = []) {
    return (Array.isArray(intervals) ? intervals : [])
      .map((item) => {
        const inicio = clamp(toNumber(item?.parcela_inicio, 1), 1, CREDITUR_PRE_DEFAULT_MONTHS);
        const fim = clamp(toNumber(item?.parcela_fim, inicio), 1, CREDITUR_PRE_DEFAULT_MONTHS);
        const valor = roundMoneyNumber(parseMoney(item?.valor || 0));
        if (valor <= 0) return null;
        return fim < inicio
          ? { parcela_inicio: fim, parcela_fim: inicio, valor }
          : { parcela_inicio: inicio, parcela_fim: fim, valor };
      })
      .filter(Boolean)
      .sort((a, b) => a.parcela_inicio - b.parcela_inicio || a.parcela_fim - b.parcela_fim);
  }

  function getCredituGeralManual7lmIntervals(result = {}) {
    if (!isCredituGeralManual7lm(result)) return [];
    const storedIntervals = (Array.isArray(result?.parcelas_creditur_intervalos) ? result.parcelas_creditur_intervalos : [])
      .map((item) => {
        const rawInicio = clamp(toNumber(item?.parcela_inicio, 1), 1, CREDITUR_TOTAL_MAX_MONTHS);
        const rawFim = clamp(toNumber(item?.parcela_fim, rawInicio), 1, CREDITUR_TOTAL_MAX_MONTHS);
        const start = Math.max(Math.min(rawInicio, rawFim), CREDITUR_PRE_DEFAULT_MONTHS + 1);
        const end = Math.min(Math.max(rawInicio, rawFim), CREDITUR_TOTAL_MAX_MONTHS);
        const valor = roundMoneyNumber(parseMoney(item?.valor || 0));
        if (valor <= 0 || end < start || isCredituGeralCrediturInstallmentValue(result, valor)) return null;
        return { parcela_inicio: start, parcela_fim: end, valor };
      })
      .filter(Boolean)
      .sort((a, b) => a.parcela_inicio - b.parcela_inicio || a.parcela_fim - b.parcela_fim);
    if (storedIntervals.length) return storedIntervals;
  ...
  function buildCredituGeralDisplayIntervals(result = state.credituGeral) {
    ...
    const credituIntervals = buildCredituGeralPayloadIntervals({
      ...base,
      aplicado: true,
      valor_liberado: parseMoney(base.valor_liberado || 0),
      parcelas_creditur_intervalos: sanitizeCredituGeralPdfIntervals(base.parcelas_creditur_intervalos),
      parcela_7lm_20: 0,
      parcela_7lm_manual: false,
    }, []);
    const displayIntervals = normalizeCrediturIntervalList([
      ...credituIntervals,
      ...getCredituGeralManual7lmIntervals(base),
    ]);
    if (!displayIntervals.some((item) => normalizeCrediturPhase(item.fase) === CREDITUR_PHASE_POST)) {
      displayIntervals.push({
        fase: CREDITUR_PHASE_POST,
        parcela_inicio: 1,
        parcela_fim: getCrediturConfiguredPostMonths(),
        valor: "",
      });
    }
  ...
  function syncCredituGeral7lmFromCurve() {
    if (!isCrediturGeralSelected() || !isCredituGeralPdfResult(state.credituGeral)) return;
    const current = collectCrediturSemestres({ normalize: true, includeEmpty: true });
    const parcela7lm = buildCredituGeral7lmIntervals(state.credituGeral, current)[0]?.valor || 0;
    state.credituGeral = {
      ...state.credituGeral,
      parcela_7lm_20: parcela7lm,
      parcela_7lm_manual: parcela7lm > 0,
      parcelas_creditur_intervalos: buildCredituGeralPayloadIntervals(state.credituGeral, current),
    };
  }
  ...
  function compactFileNameForStatus(name, maxLength = 34) {
    ...
    const headLength = Math.max(12, maxLength - extension.length - 12);
    const tailLength = Math.max(6, maxLength - extension.length - headLength - 3);
    return `${base.slice(0, headLength)}...${base.slice(-tailLength)}${extension}`;
  }

  function buildAttachmentStatusMessage(fileName, suffix, maxLength = 34) {
    const compactName = compactFileNameForStatus(fileName, maxLength);
    return compactName ? `${compactName} ${suffix}` : suffix;
  }

  function setCredituGeralPdfStatus(text, variant = "", title = "") {
    if (!el.credituGeralPdfStatus) return;
    const message = text || "Nenhum PDF carregado.";
    el.credituGeralPdfStatus.textContent = message;
    el.credituGeralPdfStatus.title = title || message;
  ...
  function setCredituGeralUploadStatus(node, text, variant = "", fallback = "Nenhum anexo selecionado.", title = "") {
    if (!node) return;
  ...
  function setCredituGeralSerasaStatus(text, variant = "", title = "") {
    setCredituGeralUploadStatus(el.credituGeralSerasaStatus, text, variant, "Nenhum score anexado.", title);
  }

  function setCredituGeralSicaqStatus(text, variant = "", title = "") {
    setCredituGeralUploadStatus(el.credituGeralSicaqStatus, text, variant, "Nenhum SICAQ anexado.", title);
  }
  ...
  function normalizeCredituGeralPdfResult(payload = {}) {
    ...
    return {
      ...
      // A parcela 7LM e complementar ao PDF da Creditu e deve ser preenchida manualmente.
      parcela_7lm_20: 0,
      parcela_7lm_manual: false,
      prazo,
      ...
      parcelas_creditur_intervalos: sanitizeCredituGeralPdfIntervals(campos.parcelas_creditur_intervalos),
    };
  }
  ...
  async function importCredituGeralPdf() {
    ...
    try {
      ...
      syncCredituGeralApplyState();
      const importedPdfName = payload.arquivo?.nome_original || file.name;
      const importedPdfStatus = buildAttachmentStatusMessage(
        importedPdfName,
        `carregado. Valor Creditú ${formatMoney(result.valor_liberado)}.`
      );
      setCredituGeralPdfStatus(
        importedPdfStatus,
        "success",
        `${importedPdfName} carregado. Valor Creditú ${formatMoney(result.valor_liberado)}.`
      );
      setCredituGeralSerasaStatus(
        result.anexo_serasa?.nome_original
          ? buildAttachmentStatusMessage(result.anexo_serasa.nome_original, "anexado.")
          : "Score Serasa não anexado.",
        result.anexo_serasa?.nome_original ? "success" : "",
        result.anexo_serasa?.nome_original ? `${result.anexo_serasa.nome_original} anexado.` : ""
      );
      setCredituGeralSicaqStatus(
        result.anexo_sicaq?.nome_original
          ? buildAttachmentStatusMessage(result.anexo_sicaq.nome_original, "anexado.")
          : "SICAQ não anexado.",
        result.anexo_sicaq?.nome_original ? "success" : "",
        result.anexo_sicaq?.nome_original ? `${result.anexo_sicaq.nome_original} anexado.` : ""
      );
  ...
  function renderCredituGeralResult(result = state.credituGeral || getCredituGeralEmptyResult()) {
    ...
    if (el.credituGeralDetalhes) {
      ...
      const rows = [
        ...
        ["7LM Complementar", formatMoney(getCredituGeralParcela7lm(result))],
  ...
  function applyCredituGeralSimulation() {
    ...
    showFeedback("success", `PDF Creditú Aplicado: ${formatMoney(state.credituGeral.valor_liberado)} No Pré-Chaves. Preencha A 7LM Complementar Se Necessário.`);
  ...
  function renderCrediturSemestres({ preserve = true } = {}) {
    ...
    state.crediturSemestres.forEach((item, index) => {
      ...
      inicioInput.disabled = credituGeral && isCrediturRow;
      ...
      fimInput.disabled = credituGeral && isCrediturRow;
      ...
      valorLabel.textContent = isCrediturRow ? "Parcela mensal Creditú" : "Parcela mensal 7LM Complementar";
      ...
      valorInput.readOnly = credituGeral && isCrediturRow;
      valorInput.disabled = false;
      valorInput.setAttribute("aria-readonly", credituGeral && isCrediturRow ? "true" : "false");
  ...
  function buildSimulationPayload() {
    ...
    if (parceiroSimulacao === PARTNER_CREDITUR_GERAL) {
      syncCredituGeral7lmFromCurve();
    }
...
<body>
  ...
  async function runCalculation({ silent = false, autoOpenNotifications = false } = {}) {
    ...
    renderSuggestionsSummary(getVisibleSuggestions());
    renderCompareTray();
    renderPropertyPreview();
  ...
  async function runPrimaryCalculation() {
      ...
      await runSuggestions({
        ...
        preserveSelection: false,
        source: "escolha-inicial",
        limit: LIMITE_SUGESTOES_CALCULO_SEM_IMOVEL,
      });
  ...
  function bindEvents() {
    ...
    el.credituGeralPdf?.addEventListener("change", () => {
      ...
      if (file) {
        state.credituGeralPdfPendente = true;
        setCredituGeralPdfStatus(
          buildAttachmentStatusMessage(file.name, "selecionado. Clique em Importar proposta."),
          "warning",
          `${file.name} selecionado. Clique em Importar proposta.`
        );
        ...
        state.credituGeralPdfPendente = false;
        const loadedPdfName = state.credituGeral?.arquivo_pdf?.nome_original || "PDF Creditú";
        setCredituGeralPdfStatus(
          isCredituGeralPdfResult(state.credituGeral)
            ? buildAttachmentStatusMessage(loadedPdfName, "carregado.")
            : "Anexe o PDF oficial da Creditú para liberar a aplicação.",
          isCredituGeralPdfResult(state.credituGeral) ? "success" : "warning",
          isCredituGeralPdfResult(state.credituGeral) ? `${loadedPdfName} carregado.` : ""
        );
      }
      syncCredituGeralApplyState();
    });
    ...
    el.credituGeralSerasa?.addEventListener("change", () => {
      ...
      if (file) {
        setCredituGeralSerasaStatus(
          buildAttachmentStatusMessage(file.name, "selecionado. Será enviado junto com o PDF."),
          "warning",
          `${file.name} selecionado. Será enviado junto com o PDF.`
        );
      } else {
        setCredituGeralSerasaStatus("Nenhum score anexado.");
      }
