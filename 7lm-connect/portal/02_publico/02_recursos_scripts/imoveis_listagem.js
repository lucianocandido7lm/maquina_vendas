(function () {
  ...
  function toggleModal(section, open) {
    ...
    const hasOpen = [el.importSection, el.showroomSection, el.mapaSection, el.comparacaoSection].some((x) => x && !x.hidden);
      ...
      <article class="tl-imoveis-showcase-card" data-id="${id}" tabindex="0" role="button" aria-label="Abrir showroom de ${esc(item.titulo || "Imovel")}">
        ...
        <div class="tl-imoveis-showcase-card__body">
          ...
          <div class="tl-imoveis-showcase-card__meta">
            ...
            <span><b>${esc(String(num(item.banheiros) ?? "-"))}</b><small>Banheiros</small></span>
  ...
  function openLightbox(midias, idx) {
    ...
    const show = (x) => {
      ...
      img.src = pth(fotos[i].caminho_arquivo);
      img.alt = fotos[i].nome_arquivo || "Midia";
  ...
  function infoShowroom(item) {
    ...
    const descricaoCompacta = compact(text(item.descricao) || "Sem descricao comercial cadastrada para esta unidade.", 184);
    ...
    return `
      ...
      <div class="tl-imoveis-showroom__price-row"><strong>${shared.formatCurrency(item.valor)}</strong><span class="tl-imoveis-badge tl-imoveis-badge--status tl-imoveis-badge--${esc(sKey(item.status))}">${esc(sLabel(item.status))}</span></div>
      <div class="tl-imoveis-showroom__meta-grid">
        <article><span>Unidade</span><strong>${esc(d.unidade || item?.agrupamento?.unidade || "-")}</strong></article>
        <article><span>Bloco</span><strong>${esc(d.bloco || item?.agrupamento?.bloco || "-")}</strong></article>
        <article><span>Pavimento</span><strong>${esc(d.pavimento || item?.agrupamento?.pavimento || "-")}</strong></article>
        <article><span>Quartos</span><strong>${esc(String(num(item.quartos) ?? "-"))}</strong></article>
        <article><span>Banheiros</span><strong>${esc(String(num(item.banheiros) ?? "-"))}</strong></article>
        ...
        <article><span>Area total</span><strong>${esc(areaTotal)}</strong></article>
        <article><span>Orientacao</span><strong>${esc(text(d.orientacao) || "-")}</strong></article>
        <article><span>Posicao</span><strong>${esc(text(d.posicao) || "-")}</strong></article>
      </div>
      <div class="tl-imoveis-showroom__location">
        <div class="tl-imoveis-showroom__location-text">
          ...
          <strong title="${esc(enderecoCompleto)}">${esc(compact(enderecoCompleto, 116))}</strong>
          <small title="${esc(localCompacto)}">${esc(compact(localCompacto, 44))}</small>
        </div>
        <button class="tl-imoveis-btn tl-imoveis-btn--secondary" type="button" data-action="show-map">Ver no mapa</button>
      </div>
      <div class="tl-imoveis-showroom__highlights"><span class="tl-imoveis-eyebrow mono-font">DIFERENCIAIS</span>${difs.length ? `<div class="tl-imoveis-showroom__tags">${difs.map((x) => `<span class="tl-imoveis-chip">${esc(x)}</span>`).join("")}</div>` : "<p>Nenhum diferencial comercial cadastrado.</p>"}</div>
      <div class="tl-imoveis-showroom__description"><span class="tl-imoveis-eyebrow mono-font">DESCRICAO</span><p>${esc(descricaoCompacta)}</p></div>
      <div class="tl-imoveis-showroom__timeline"><span class="tl-imoveis-eyebrow mono-font">RESUMO RAPIDO</span><ul><li><strong>Status:</strong> ${esc(sLabel(item.status))}</li><li><strong>Disponibilidade:</strong> ${esc(sLabel(item.status) === "Disponivel" ? "Pronto para proposta" : sLabel(item.status))}</li><li><strong>Cadastro:</strong> ${esc(shared.formatDate(item.data_hora_criacao) || "-")}</li><li><strong>Atualizacao:</strong> ${esc(shared.formatDate(item.data_hora_atualizado_em) || "-")}</li></ul></div>
      <div class="tl-imoveis-showroom__actions">
        <button class="tl-imoveis-btn" type="button" data-action="show-simular">Simular com este imovel</button>
        ...
        <button class="tl-imoveis-btn tl-imoveis-btn--secondary" type="button" data-action="show-close">Fechar showroom</button>
      </div>`;
  }
    ...
    if (el.showroomContent) {
      el.showroomContent.innerHTML = `
        <div class="tl-imoveis-showroom__layout">
          ...
          <section class="tl-imoveis-showroom__details">${infoShowroom(item)}</section>
        </div>`;
    }
    renderCarousel();
  }
  ...
  function mapLinks(item) {
    ...
    const q = text(locx.consulta || addr);
    const enc = encodeURIComponent(q);
    return {
      endereco: addr,
      cidade: [locx.bairro || item?.bairro, locx.cidade || item?.cidade, locx.estado || item?.estado].filter(Boolean).join(" - "),
      embed: locx.google_maps_embed_url || (q ? `https://www.google.com/maps?q=${enc}&output=embed` : ""),
      maps: locx.google_maps_url || (q ? `https://www.google.com/maps/search/?api=1&query=${enc}` : ""),
      rota: locx.google_maps_rota_url || (q ? `https://www.google.com/maps/dir/?api=1&destination=${enc}` : ""),
    };
  }
  ...
  async function openComparacao() {
    ...
    el.comparacaoContent.innerHTML = `<div class="tl-imoveis-compare-grid">${items.map((i) => {
      ...
      return `<article class="tl-imoveis-compare-card"><div class="tl-imoveis-compare-card__media">${f ? `<img src="${esc(f)}" alt="${esc(i.titulo || "Imovel")}" loading="lazy" />` : `<div class="tl-imoveis-showcase-card__hero tl-imoveis-showcase-card__hero--empty"><span>Sem foto</span></div>`}<span class="tl-imoveis-commercial-badge">${esc(badge)}</span></div><div class="tl-imoveis-compare-card__body"><h3>${esc(i.titulo || "Imovel")}</h3><p>${esc(local(i))}</p><strong>${shared.formatCurrency(i.valor)}</strong><span class="tl-imoveis-badge tl-imoveis-badge--status tl-imoveis-badge--${esc(sKey(i.status))}">${esc(sLabel(i.status))}</span><ul><li><b>Tipo:</b> ${esc(i.tipo_imovel || "-")}</li><li><b>Quartos:</b> ${esc(String(num(i.quartos) ?? "-"))}</li><li><b>Banheiros:</b> ${esc(String(num(i.banheiros) ?? "-"))}</li><li><b>Vagas:</b> ${esc(String(num(i.vagas_garagem) ?? "-"))}</li><li><b>Area:</b> ${esc(area(i.area_m2))}</li></ul>${dif.length ? `<div class="tl-imoveis-compare-card__tags">${dif.map((d) => `<span class="tl-imoveis-chip">${esc(d)}</span>`).join("")}</div>` : ""}<div class="tl-imoveis-compare-card__actions"><button class="tl-imoveis-btn" type="button" data-action="open-showroom" data-id="${esc(i.id)}">Abrir showroom</button></div></div></article>`;
    }).join("")}</div>`;
  }
  ...
  function bindEvents() {
    ...
    el.btnLimpar?.addEventListener("click", () => {
      ...
      if (el.inputStatus) el.inputStatus.value = "";
      state.pagina = 1;
      void carregarImoveis();
    });
