...
async def resumir_vinculos_exclusao_cliente(conexao, esquema: str, identificador_cliente: str):
    return await conexao.fetchrow(
        f"""
        select
            (
            ...
            )::integer as reservas_ativas,
            (
                select count(*)
                  from {esquema}.imovel_reserva r
                 where r.identificador_cliente = $1::uuid
                   and r.status = 'CONVERTIDA'
            )::integer as reservas_convertidas,
            (
                select count(*)
                  from {esquema}.imovel_reserva r
                  join {esquema}.imovel i
                    on i.identificador_imovel = r.identificador_imovel
                 where r.identificador_cliente = $1::uuid
                   and lower(trim(coalesce(i.status, ''))) = 'vendido'
            )::integer as imoveis_vendidos,
            (
                select count(*)
                  from {esquema}.historico_status_imovel h
                 where h.identificador_cliente = $1::uuid
                   and lower(trim(coalesce(h.status_novo, ''))) = 'vendido'
            )::integer as vendas_historico
        """,
        identificador_cliente,
    )
...
async def excluir_cliente(conexao, esquema: str, identificador_cliente: str):
    reservas_ativas = await conexao.fetch(
        f"""
        select distinct r.identificador_imovel::text as identificador_imovel
          from {esquema}.imovel_reserva r
         where r.identificador_cliente = $1::uuid
           and r.status in ('ATIVA', 'PENDENTE_APROVACAO')
        """,
    ...
    imoveis_para_liberar = [str(item["identificador_imovel"]) for item in reservas_ativas]
    if imoveis_para_liberar:
        await conexao.execute(
            f"""
            update {esquema}.imovel
               set status = 'Disponivel'
             where identificador_imovel = any($1::uuid[])
               and lower(trim(coalesce(status, ''))) not in ('vendido', 'inativo')
            """,
            imoveis_para_liberar,
        )

    await conexao.execute(
        f"delete from {esquema}.aprovacao_excecao where identificador_cliente = $1::uuid",
        identificador_cliente,
    )
    await conexao.execute(
        f"delete from {esquema}.historico_status_imovel where identificador_cliente = $1::uuid",
        identificador_cliente,
    )
    await conexao.execute(
        f"delete from {esquema}.imovel_reserva where identificador_cliente = $1::uuid",
        identificador_cliente,
    )
    await conexao.execute(
        f"delete from {esquema}.simulacao where identificador_cliente = $1::uuid",
        identificador_cliente,
    )
    return await conexao.fetchrow(
        f"""
