"""
Esquemas Pydantic do modulo de imóveis.
"""
...
class RequisicaoImovelBase(BaseModel):
    ...
    valor_desconto_minimo: Optional[Decimal] = Field(0, ge=0)
    valor_incentivo_minimo: Optional[Decimal] = Field(None, ge=0)
    valor_desconto_maximo: Optional[Decimal] = Field(50000, ge=0)
    valor_incentivo_maximo: Optional[Decimal] = Field(None, ge=0)
...
class RequisicaoRegistrarEvolucaoObra(BaseModel):
    ...
    data_referencia: Optional[date] = Field(
        None,
        validation_alias=AliasChoices("data_referencia", "data_referência"),
    )
    observacoes: Optional[str] = Field(
        None,
        max_length=500,
        validation_alias=AliasChoices("observacoes", "observações"),
    )
    identificadores_imoveis: Optional[list[str]] = None
