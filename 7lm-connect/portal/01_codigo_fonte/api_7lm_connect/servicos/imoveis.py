...
from reportlab.platypus import Flowable, Image as PlatypusImage, KeepTogether, LongTable, PageBreak, Paragraph, SimpleDocTemplate, Spacer, Table, TableStyle
...
DESCONTO_IMOVEL_MAXIMO_PADRAO = Decimal("50000.00")
DESCONTO_IMOVEL_REDUCAO_FAIXA = Decimal("5000.00")
DESCONTO_IMOVEL_UNIDADES_POR_FAIXA = 6000


def _decimal_positivo(valor: Any, padrao: Decimal = Decimal("0.00")) -> Decimal:
    if valor in (None, ""):
        return padrao
    try:
...
def _politica_desconto_imovel(registro: dict[str, Any]) -> dict[str, Decimal | int]:
    minimo = _decimal_positivo(registro.get("valor_desconto_minimo"), DESCONTO_IMOVEL_MINIMO_PADRAO)
    maximo_configurado = _decimal_positivo(registro.get("valor_desconto_maximo"), DESCONTO_IMOVEL_MAXIMO_PADRAO)
    if maximo_configurado < minimo:
        maximo_configurado = minimo

    quantidade = _inteiro_positivo(registro.get("quantidade_desconto_reservas_vendas"))
    reducao = Decimal(quantidade // DESCONTO_IMOVEL_UNIDADES_POR_FAIXA) * DESCONTO_IMOVEL_REDUCAO_FAIXA
    maximo_efetivo = max(Decimal("0.00"), maximo_configurado - reducao)
    return {
        "minimo": minimo,
        "maximo_configurado": maximo_configurado,
        "maximo_efetivo": maximo_efetivo,
        "reducao": reducao,
...
def _montar_dados_agrupamento_imovel(registro: dict[str, Any]) -> dict[str, Any]:
    ...
    descricao = _texto_limpo(_valor_chave(registro, "descricao", "descrição"))
    endereco = _texto_limpo(_valor_chave(registro, "endereco", "endereço"))
...
class _ImagemCover(Flowable):
    """Desenha a imagem preenchendo toda a área do card, com recorte central."""

    def __init__(self, caminho: Path | str, largura: float, altura: float):
        super().__init__()
        self._image = ImageReader(str(caminho))
        self.width = max(float(largura), 1.0)
        self.height = max(float(altura), 1.0)

    def wrap(self, availWidth: float, availHeight: float) -> tuple[float, float]:
        return self.width, self.height

    def draw(self) -> None:
        largura_original, altura_original = self._image.getSize()
        if not largura_original or not altura_original:
            return

        escala = max(self.width / largura_original, self.height / altura_original)
        largura_desenho = largura_original * escala
        altura_desenho = altura_original * escala
        deslocamento_x = (self.width - largura_desenho) / 2
        deslocamento_y = (self.height - altura_desenho) / 2

        self.canv.saveState()
        caminho_recorte = self.canv.beginPath()
        caminho_recorte.rect(0, 0, self.width, self.height)
        self.canv.clipPath(caminho_recorte, stroke=0, fill=0)
        self.canv.drawImage(
            self._image,
            deslocamento_x,
            deslocamento_y,
...
def _montar_endereco_formatado(registro: dict[str, Any]) -> str:
    partes = [
        _texto_limpo(_valor_chave(registro, "endereco", "endereço")),
...
def _montar_detalhes_comerciais(registro: dict[str, Any], agrupamento: dict[str, Any]) -> dict[str, Any]:
    descricao = _texto_limpo(_valor_chave(registro, "descricao", "descrição"))
    area_privativa = _extrair_campo_rotulado(descricao, "área privativa")
    area_total = (
        _extrair_campo_rotulado(descricao, "área total construida")
        or _extrair_campo_rotulado(descricao, "área total")
    )
...
def _montar_diferenciais_comerciais(registro: dict[str, Any], detalhes: dict[str, Any]) -> list[str]:
    ...
    descricao = _texto_limpo(_valor_chave(registro, "descricao", "descrição")).lower()
...
def serializar_evolucao_obra(linha: dict[str, Any] | Any) -> dict[str, Any]:
    registro = dict(linha)
    data_referencia = _data_para_iso(_valor_chave(registro, "data_referencia", "data_referência"))
    observacoes = _valor_chave(registro, "observacoes", "observações")
    return {
...
def serializar_imovel(
    ...
    *,
    ...
    politica_desconto = _politica_desconto_imovel(registro)

    return {
        "id": str(registro["identificador_imovel"]),
        "titulo": registro["titulo"],
        "descricao": _valor_chave(registro, "descricao", "descrição"),
        "tipo_imovel": registro.get("tipo_imovel"),
        "endereco": _valor_chave(registro, "endereco", "endereço"),
        ...
        "valor_desconto_minimo": _decimal_para_float(politica_desconto["minimo"]),
        "valor_incentivo_minimo": _decimal_para_float(politica_desconto["minimo"]),
        "valor_desconto_maximo": _decimal_para_float(politica_desconto["maximo_configurado"]),
        "valor_incentivo_maximo": _decimal_para_float(politica_desconto["maximo_configurado"]),
        "desconto_imovel_maximo_efetivo": _decimal_para_float(politica_desconto["maximo_efetivo"]),
        "incentivo_7lm_maximo_efetivo": _decimal_para_float(politica_desconto["maximo_efetivo"]),
        "desconto_imovel_reducao_por_reservas_vendas": _decimal_para_float(politica_desconto["reducao"]),
        "incentivo_7lm_reducao_por_reservas_vendas": _decimal_para_float(politica_desconto["reducao"]),
        "quantidade_desconto_reservas_vendas": politica_desconto["quantidade"],
        "quantidade_incentivo_reservas_vendas": politica_desconto["quantidade"],
        ...
        "simulacao_id": str(registro.get("ultima_simulacao_id") or "") or None,
        "ultima_simulacao_id": str(registro.get("ultima_simulacao_id") or "") or None,
...
EXPORTACAO_IMOVEIS_COLUNAS = [
    ...
    {"chave": "endereco_formatado", "rotulo": "Endereço", "largura": 42, "tipo": "texto"},
    {"chave": "valor", "rotulo": "Valor do imóvel", "largura": 16, "tipo": "moeda"},
    {"chave": "valor_garantido", "rotulo": "Valor garantido", "largura": 16, "tipo": "moeda"},
    {"chave": "valor_parcela_minima_pre_obra", "rotulo": "Pré-obra mínima", "largura": 16, "tipo": "moeda"},
    {"chave": "percentual_fechamento_minimo", "rotulo": "Captação mínima", "largura": 15, "tipo": "percentual"},
    {"chave": "data_entrega", "rotulo": "Data de entrega", "largura": 15, "tipo": "data"},
    {"chave": "meses_pre_entrega", "rotulo": "Meses até entrega", "largura": 14, "tipo": "inteiro"},
    {"chave": "meses_pos_entrega", "rotulo": "Meses pos-entrega", "largura": 14, "tipo": "inteiro"},
    {"chave": "percentual_conclusao_obra", "rotulo": "Conclusão da obra", "largura": 15, "tipo": "percentual"},
    ...
    {"chave": "area_m2", "rotulo": "área m2", "largura": 12, "tipo": "decimal"},
    {"chave": "quantidade_midias", "rotulo": "Mídias", "largura": 10, "tipo": "inteiro"},
...
def _mapear_imovel_para_exportacao(imovel: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "endereco_formatado": _primeiro_texto_exportacao(
            ...
            _valor_chave(imovel, "endereco", "endereço"),
        ),
...
def gerar_xlsx_exportacao_imoveis(imoveis: list[dict[str, Any]], filtros: dict[str, Any]) -> bytes:
    ...
    sheet["A1"] = "Relatório de imoveis cadastrados"
    ...
    metricas = [
        ("Total", resumo["total"]),
        ("Disponíveis", resumo["disponíveis"]),
...
def gerar_xlsx_exportacao_imoveis_fallback(imoveis: list[dict[str, Any]], filtros: dict[str, Any]) -> bytes:
    resumo = _resumo_exportacao_imoveis([_mapear_imovel_para_exportacao(item) for item in imoveis])
    linhas = [
        ["Relatório de imoveis cadastrados"],
        [f"Gerado em {_formatar_data_hora_br(datetime.now())}"],
        [f"Filtros: {_descricao_filtros_exportacao(filtros)}"],
        [f"Total: {resumo['total']} | Disponíveis: {resumo['disponíveis']} | Reservados: {resumo['reservados']} | Ticket medio: {_formatar_moeda_br(resumo['ticket_medio'])}"],
        [""],
...
def gerar_pdf_exportacao_imoveis(imoveis: list[dict[str, Any]], filtros: dict[str, Any]) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=12 * mm,
        bottomMargin=10 * mm,
        title="Relatório de imoveis cadastrados",
    )
    ...
    def _hero_media_card(linha: dict[str, Any], largura: float, altura: float) -> Table:
        ...
        destaque_local = _resumir_texto_exportacao(descrever_localizacao(linha), 78) or "Local não informado"
        ...
        if caminho_midia is not None:
            try:
                conteudo.append(
                    _ImagemCover(
                        caminho_midia,
                        largura=max(largura - 16, 10),
                        altura=max(altura - 16, 10),
                    )
                )
            except Exception:
                conteudo = []
    ...
    def _card_imovel(linha: dict[str, Any], largura: float) -> Table:
        ...
        metricas = Table(
            [
                [
                    _card_metrica("Valor do imóvel", _formatar_moeda_br(linha.get("valor")), (largura - 28) / 2),
                    _card_metrica("Valor garantido", _formatar_moeda_br(linha.get("valor_garantido")), (largura - 28) / 2),
                ],
                [
                    _card_metrica("Pré-obra mínima", _formatar_moeda_br(linha.get("valor_parcela_minima_pre_obra")), (largura - 28) / 2),
                    _card_metrica(
                        "Entrega",
                        " • ".join(
                            parte for parte in [
        ...
        elementos: list[Any] = [
            Table(
                [[Paragraph("IMÓVEL", estilo_card_kicker), _badge_status(status)]],
        ...
        rodape = " • ".join(
            parte for parte in [
                f"Captação mínima {_formatar_percentual_br(linha.get('percentual_fechamento_minimo'))}" if linha.get("percentual_fechamento_minimo") not in (None, "") else "",
    ...
    hero_metrics = Table(
        [
            ...
            [
                ...
                _hero_metric_card("Próxima entrega", _formatar_data_br(metricas["proxima_entrega"]), hero_metric_half),
            ],
        ],
    ...
    hero_conteudo = [
        top_row,
        Spacer(1, 5 * mm),
        Paragraph("Relatório de imoveis cadastrados", estilo_hero_titulo),
        Spacer(1, 1.2 * mm),
        Paragraph(
    ...
    def desenhar_rodape(canvas, doc_obj):
        ...
        canvas.drawRightString(doc.pagesize[0] - doc.rightMargin, 7 * mm, f"Página {canvas.getPageNumber()}")
        canvas.restoreState()
...
def gerar_pdf_exportacao_imoveis_simulado(imoveis: list[dict[str, Any]], filtros: dict[str, Any]) -> bytes:
    ...
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=9 * mm, rightMargin=9 * mm, topMargin=9 * mm, bottomMargin=9 * mm, title="Relatório de imoveis cadastrados")
    base = getSampleStyleSheet()
    ...
    def local(linha: dict[str, Any]) -> str: return " - ".join(parte for parte in [_texto_limpo(linha.get("bairro")), _texto_limpo(linha.get("cidade"))] if parte) or _texto_limpo(linha.get("endereco_formatado")) or "Local não informado"
    ...
    hero = Table([[[Paragraph("SIMULADOR VISUAL", s_kicker), Spacer(1, 1.2 * mm), Paragraph("Base de imóveis cadastrados", s_title), Spacer(1, 1.1 * mm), Paragraph("Relatório no padrao visual do simulador, com hierarquia comercial mais clara para apresentar a carteira.", s_light), Spacer(1, 2.5 * mm), Paragraph(f"Gerado em {_formatar_data_hora_br(datetime.now())}<br/>Filtros ativos: {xml_escape(_descricao_filtros_exportacao(filtros))}", s_light), Spacer(1, 3 * mm), hero_bloco, Spacer(1, 3 * mm), grade([("Valor total da carteira", _formatar_moeda_br(metricas["valor_total"])), ("Ticket medio", _formatar_moeda_br(resumo["ticket_medio"])), ("Reservas ativas", str(metricas["reservas"])), ("Próxima entrega", _formatar_data_br(metricas["proxima_entrega"]))], 2)]]], colWidths=[doc.width])
    ...
    if metricas["sem_midias"] > 0: alertas.append(f"{metricas['sem_midias']} unidade(s) ainda não possuem mídias publicadas.")
    if resumo["inativos"] > 0: alertas.append(f"{resumo['inativos']} unidade(s) estao marcadas como inativas e precisam revisao.")
    if resumo["disponíveis"] > 0: alertas.append(f"{resumo['disponíveis']} unidade(s) estão livres para simulacao e negociacao.")
    ...
    def rodape(canvas, doc_obj):
        canvas.saveState(); canvas.setFont("Helvetica", 8); canvas.setFillColor(c_muted); canvas.drawString(doc.leftMargin, 6 * mm, "7LM Connect | Exportacao comercial de imoveis"); canvas.drawRightString(doc.pagesize[0] - doc.rightMargin, 6 * mm, f"Página {canvas.getPageNumber()}"); canvas.restoreState()
    historia: list[Any] = [hero, Spacer(1, 5 * mm)]
    historia.extend(secao("Resumo comercial", "Carteira e filtros"))
    historia.append(Table([[card("Filtros", _descricao_filtros_exportacao(filtros), doc.width * 0.5 - 2), card("Distribuicao", f"Disponíveis {resumo['disponíveis']} • Reservados {resumo['reservados']} • Vendidos {resumo['vendidos']} • Inativos {resumo['inativos']} • Cidades {resumo['cidades']}", doc.width * 0.5 - 2)]], colWidths=[doc.width * 0.5, doc.width * 0.5]))
    historia.append(Spacer(1, 4 * mm))
    historia.extend(secao("Indicadores da operação", "Resumo financeiro"))
    historia.append(grade([("Valor total", _formatar_moeda_br(metricas["valor_total"])), ("Garantido total", _formatar_moeda_br(metricas["valor_garantido_total"])), ("Ticket medio", _formatar_moeda_br(metricas["valor_medio"])), ("Garantido medio", _formatar_moeda_br(metricas["valor_garantido_medio"])), ("Pré-obra mídia", _formatar_moeda_br(metricas["pre_obra_media"])), ("Conclusão mídia", f"{metricas['conclusao_obra_media']:.2f}%".replace(".", ",")), ("Mídias totais", str(metricas["total_midias"])), ("Sem mídias", str(metricas["sem_midias"]))], 4))
    historia.append(Spacer(1, 4 * mm))
    ...
    if not linhas:
        ...
        tabela_linhas: list[list[Any]] = [[Paragraph("Imovel", s_head), Paragraph("Localização", s_head), Paragraph("Status", s_head), Paragraph("Valor", s_head), Paragraph("Garantido", s_head), Paragraph("Entrega", s_head), Paragraph("% obra", s_head), Paragraph("Reserva", s_head)]]
        for linha in linhas:
            ...
            tabela_linhas.append([[p(_valor_texto_exportacao(linha, {"chave": "titulo", "tipo": "texto"}), s_cell), Spacer(1, 0.8 * mm), p(detalhes(linha), s_muted)], [p(local(linha), s_cell), Spacer(1, 0.8 * mm), p(_texto_limpo(linha.get("endereco_formatado")) or "Endereço não informado", s_muted)], p(_valor_texto_exportacao(linha, {"chave": "status", "tipo": "texto"}), s_cell), p(_formatar_moeda_br(linha.get("valor")), s_cell), p(_formatar_moeda_br(linha.get("valor_garantido")), s_cell), p(entrega, s_cell), p(_formatar_percentual_br(linha.get("percentual_conclusao_obra")), s_cell), p(reserva_texto, s_reserva if reserva_texto != "-" else s_cell)])
...
def gerar_pdf_exportacao_imoveis_simulado(imoveis: list[dict[str, Any]], filtros: dict[str, Any]) -> bytes:
    ...
    def criar_badge_status_imovel(texto: str, largura: float = 30 * mm) -> Table:
        status_normalizado = _texto_limpo(texto).lower()
        mapa = {
            "disponivel": ("#0E7C66", "#5DD6B0"),
            "reservado": ("#A05C00", "#F0C36B"),
            "vendido": ("#B42318", "#F39B95"),
            "inativo": ("#475467", "#98A2B3"),
        }
        fundo, borda_badge = mapa.get(status_normalizado, ("#2A7394", "#70A9C5"))
        estilo = ParagraphStyle(
            f"PdfImoveisHeroBadgeImovel-{status_normalizado or 'default'}",
            parent=base["BodyText"],
            fontName="Helvetica-Bold",
            fontSize=7.3,
            leading=8.4,
        ...
        pill = Table(
            [[Paragraph(xml_escape(_texto_limpo(texto).upper() or "IMÓVEL"), estilo)]],
            colWidths=[largura],
            rowHeights=[11 * mm],
        )
        pill.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor(fundo)),
            ("BOX", (0, 0), (-1, -1), 0.7, colors.HexColor(borda_badge)),
            ("LEFTPADDING", (0, 0), (-1, -1), 8),
            ("RIGHTPADDING", (0, 0), (-1, -1), 8),
            ("TOPPADDING", (0, 0), (-1, -1), 3),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 3),
    ...
    def criar_linha_cards(
        campos: list[tuple[str, str]],
        larguras: list[float],
        *,
        padding_horizontal: float = 6.5,
        padding_vertical: float = 5.5,
    ) -> Table:
        cards: list[Any] = []
        for indice, largura in enumerate(larguras):
            if indice < len(campos):
                rotulo, valor = campos[indice]
                cards.append(
                    criar_card_info(
                        rotulo,
                        valor,
                ...
                vazio = Table([[""]], colWidths=[largura - 2])
                vazio.setStyle(TableStyle([("BOX", (0, 0), (-1, -1), 0, colors.white)]))
                cards.append(vazio)
        linha = Table([cards], colWidths=larguras)
    ...
    def criar_box_aviso(texto: str, *, fundo: str, borda_box: str, cor_texto_box: str) -> Table:
        estilo_aviso = ParagraphStyle(
            f"PdfImoveisAviso-{borda_box}",
            parent=base["BodyText"],
            fontName="Helvetica-Bold",
            fontSize=7.4,
            leading=9,
            textColor=colors.HexColor(cor_texto_box),
        )
        box = Table(
            [[[paragrafo_seguro(texto, estilo_aviso, preservar_quebras=False)]]],
            colWidths=[doc.width - 24],
        )
        box.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor(fundo)),
            ("BOX", (0, 0), (-1, -1), 0.65, colors.HexColor(borda_box)),
    ...
    def criar_ficha_imovel(
        ...
        *,
        ...
        titulo = _valor_texto_exportacao(linha, {"chave": "titulo", "tipo": "texto"})
        status = _valor_texto_exportacao(linha, {"chave": "status", "tipo": "texto"})
        localizacao = descrever_localizacao(linha)
        detalhes = descrever_detalhes(linha)
        endereco = _resumir_texto_exportacao(linha.get("endereco_formatado"), 88) or "Endereço não informado"
        reserva_texto = _valor_texto_exportacao(linha, {"chave": "reserva_cliente", "tipo": "texto"})
        atualizado_em = _formatar_data_hora_br(linha.get("data_hora_atualizado_em"))
        midias_publicadas = _formatar_inteiro(linha.get("quantidade_midias"))
        badge_resolvido = _texto_limpo(badge_texto) or status
        titulo_relatorio = "Relatório da carteira de imoveis" if _texto_limpo(badge_texto) else "Relatório do imóvel"
        subtitulo_relatorio = (
            f"Unidade em destaque | {localização} | {detalhes}"
            if _texto_limpo(badge_texto)
            else f"Unidade {indice} de {total} | {localização} | {detalhes}"
        )
        top_row_imovel = Table(
            [[criar_badge_logo_hero(), criar_badge_status_imovel(badge_resolvido, largura=34 * mm)]],
            colWidths=[40 * mm, hero_largura_util - (40 * mm)],
        )
        top_row_imovel.setStyle(TableStyle([
            ("LEFTPADDING", (0, 0), (-1, -1), 0),
            ("RIGHTPADDING", (0, 0), (-1, -1), 0),
            ("TOPPADDING", (0, 0), (-1, -1), 0),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 0),
        ...
        hero_media_width_imovel = hero_largura_util * 0.56
        hero_metric_width_total_imovel = hero_largura_util - hero_media_width_imovel
        hero_metric_half_imovel = (hero_metric_width_total_imovel - 6) / 2
        hero_metrics_imovel = Table(
            [
                [
                    criar_card_metric_hero("Valor do imóvel", _formatar_moeda_br(linha.get("valor")), hero_metric_width_total_imovel, destaque=True),
                    "",
                ],
                [
                    criar_card_metric_hero("Garantido planejado", _formatar_moeda_br(linha.get("valor_garantido")), hero_metric_half_imovel),
                    criar_card_metric_hero("Pré-obra mínima", _formatar_moeda_br(linha.get("valor_parcela_minima_pre_obra")), hero_metric_half_imovel),
                ],
                [
                    criar_card_metric_hero("Entrega prevista", _formatar_data_br(linha.get("data_entrega")), hero_metric_half_imovel),
                    criar_card_metric_hero("Obra concluida", _formatar_percentual_br(linha.get("percentual_conclusao_obra")), hero_metric_half_imovel),
                ],
            ],
            colWidths=[hero_metric_half_imovel, hero_metric_half_imovel],
        )
        hero_metrics_imovel.setStyle(TableStyle([
            ("SPAN", (0, 0), (1, 0)),
        ...
        hero_grid_imovel = Table(
            [[criar_card_midia_hero(linha, hero_media_width_imovel, 70 * mm), hero_metrics_imovel]],
            colWidths=[hero_media_width_imovel, hero_metric_width_total_imovel],
        )
        hero_grid_imovel.setStyle(TableStyle([
            ("LEFTPADDING", (0, 0), (-1, -1), 0),
            ("RIGHTPADDING", (0, 0), (-1, -1), 0),
            ("TOPPADDING", (0, 0), (-1, -1), 0),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 0),
        ...
        hero_conteudo_imovel = [
            top_row_imovel,
            Spacer(1, 4.8 * mm),
            paragrafo_seguro(titulo_relatorio, estilo_hero_titulo, preservar_quebras=False),
            Spacer(1, 1.1 * mm),
            paragrafo_seguro(
                f"{_resumir_texto_exportacao(titulo, 58) or 'Unidade comercial'} - Emitido em {data_emissao}",
                estilo_hero_subtitulo,
        ...
        hero_imovel = Table([[hero_conteudo_imovel]], colWidths=[doc.width])
        hero_imovel.setStyle(TableStyle([
        ...
        resumo_linhas = [
            criar_linha_cards(
                [
                    ("Imovel", _resumir_texto_exportacao(titulo, 34) or "-"),
                    ("Status", status),
                    ("Tipo", _valor_texto_exportacao(linha, {"chave": "tipo_imovel", "tipo": "texto"})),
                    ("Localização", _resumir_texto_exportacao(localizacao, 34) or "-"),
                ],
            ...
            criar_linha_cards(
                [
                    ("Endereço", endereco),
                    ("Detalhes", _resumir_texto_exportacao(detalhes, 60) or "-"),
                ],
                [doc.width * 0.54, doc.width * 0.46],
            ),
            Spacer(1, 2 * mm),
            criar_linha_cards(
                [
                    ("Valor garantido", _formatar_moeda_br(linha.get("valor_garantido"))),
                    ("Pré-obra mínima", _formatar_moeda_br(linha.get("valor_parcela_minima_pre_obra"))),
                    ("% Captação mínima", _formatar_percentual_br(linha.get("percentual_fechamento_minimo"))),
                    ("% Obra concluida", _formatar_percentual_br(linha.get("percentual_conclusao_obra"))),
                ],
                [coluna, coluna, coluna, coluna],
            ),
            Spacer(1, 2 * mm),
            criar_linha_cards(
                [
                    ("Entrega prevista", _formatar_data_br(linha.get("data_entrega"))),
                    ("Meses até entrega", _formatar_inteiro(linha.get("meses_pre_entrega"))),
                    ("Meses pos-entrega", _formatar_inteiro(linha.get("meses_pos_entrega"))),
                    ("Mídias publicadas", midias_publicadas),
                ],
                [coluna, coluna, coluna, coluna],
            ),
            Spacer(1, 2 * mm),
            criar_linha_cards(
                [
                    ("área", f"{_formatar_numero_br(linha.get('area_m2'), 2)} m2" if linha.get("area_m2") not in (None, "") else "-"),
                    ("Quartos", _formatar_inteiro(linha.get("quartos"))),
                    ("Banheiros", _formatar_inteiro(linha.get("banheiros"))),
                    ("Vagas", _formatar_inteiro(linha.get("vagas_garagem"))),
                ],
                [coluna, coluna, coluna, coluna],
            ),
            Spacer(1, 2 * mm),
            criar_linha_cards(
                [
                    ("Garagem", _valor_texto_exportacao(linha, {"chave": "tipo_garagem", "tipo": "texto"})),
                    ("Reserva ativa", reserva_texto),
                    ("Atualizado em", atualizado_em),
                ],
                [doc.width / 3, doc.width / 3, doc.width / 3],
            ),
        ]

        alertas_ficha: list[Any] = []
        if reserva_texto not in {"", "-"}:
            alertas_ficha.append(
                criar_box_aviso(
                    f"Reserva ativa vinculada a {reserva_texto}.",
                    fundo="#FFF8EC",
                    borda_box="#F4D4A4",
                    cor_texto_box="#9A6400",
                )
            )
        if (_inteiro_seguro(linha.get("quantidade_midias")) or 0) <= 0:
            alertas_ficha.append(
                criar_box_aviso(
                    "Mídia principal ainda não foi publicada para esta unidade.",
                    fundo="#EDF7FF",
                    borda_box="#CAE4F7",
                    cor_texto_box="#235B7D",
                )
            )

        resumo_conteudo: list[Any] = [
            paragrafo_seguro("RESUMO COMERCIAL", estilo_secao_rotulo, preservar_quebras=False),
            Spacer(1, 0.6 * mm),
            paragrafo_seguro("Unidade e operação", estilo_secao_titulo, preservar_quebras=False),
            Spacer(1, 2.2 * mm),
            *resumo_linhas,
        ]
        if alertas_ficha:
            resumo_conteudo.append(Spacer(1, 2 * mm))
            for indice_alerta, alerta_box in enumerate(alertas_ficha):
                resumo_conteudo.append(alerta_box)
                if indice_alerta != len(alertas_ficha) - 1:
                    resumo_conteudo.append(Spacer(1, 1.6 * mm))

        resumo_box = Table([[resumo_conteudo]], colWidths=[doc.width])
        ...
        return [hero_imovel, Spacer(1, 4 * mm), resumo_box]
    ...
    hero_metrics = Table(
        [
            ...
            [
                ...
                criar_card_metric_hero("Próxima entrega", _formatar_data_br(metricas["proxima_entrega"]), hero_metric_half),
            ],
        ],
        colWidths=[hero_metric_half, hero_metric_half],
    )
    ...
    hero_conteudo = [
        top_row,
        Spacer(1, 4.8 * mm),
        paragrafo_seguro("Relatório da carteira de imoveis", estilo_hero_titulo, preservar_quebras=False),
        Spacer(1, 1.1 * mm),
        paragrafo_seguro(
    ...
    distribuicao = (
        f"Disponíveis {resumo['disponíveis']} | "
    ...
    if metricas["sem_midias"] > 0:
        alertas.append(f"{metricas['sem_midias']} unidade(s) ainda não possuem mídias publicadas.")
    if resumo["inativos"] > 0:
        alertas.append(f"{resumo['inativos']} unidade(s) estao marcadas como inativas e precisam de revisao.")
    if resumo["disponíveis"] > 0:
        alertas.append(f"{resumo['disponíveis']} unidade(s) estão livres para simulacao e negociacao.")
    if not alertas:
    ...
    def rodape(canvas, doc_obj):
        ...
        canvas.drawRightString(doc.pagesize[0] - doc.rightMargin, 6 * mm, f"Página {canvas.getPageNumber()}")
        canvas.restoreState()

    resumo_primeira_pagina_conteudo: list[Any] = [
        paragrafo_seguro("RESUMO COMERCIAL", estilo_secao_rotulo, preservar_quebras=False),
        Spacer(1, 0.8 * mm),
        paragrafo_seguro("Carteira e unidade em destaque", estilo_secao_titulo, preservar_quebras=False),
        Spacer(1, 2.2 * mm),
        criar_linha_cards(
            [
                ("Filtros", _descricao_filtros_exportacao(filtros)),
                ("Distribuicao", distribuicao),
            ],
            [doc.width * 0.52, doc.width * 0.48],
        ...
        criar_linha_cards(
            [
                ("Valor total da carteira", _formatar_moeda_br(metricas["valor_total"])),
                ("Garantido total", _formatar_moeda_br(metricas["valor_garantido_total"])),
                ("Ticket medio", _formatar_moeda_br(metricas["valor_medio"])),
                ("Reservas ativas", _formatar_inteiro(metricas["reservas"])),
            ],
            [doc.width / 4, doc.width / 4, doc.width / 4, doc.width / 4],
            padding_horizontal=7,
            padding_vertical=6,
        ),
        Spacer(1, 2 * mm),
        criar_linha_cards(
            [
                ("Disponíveis", _formatar_inteiro(resumo["disponíveis"])),
                ("Reservados", _formatar_inteiro(resumo["reservados"])),
                ("Sem mídias", _formatar_inteiro(metricas["sem_midias"])),
                ("Próxima entrega", _formatar_data_br(metricas["proxima_entrega"])),
            ],
            [doc.width / 4, doc.width / 4, doc.width / 4, doc.width / 4],
            padding_horizontal=7,
            padding_vertical=6,
    ...
    if alertas:
        resumo_primeira_pagina_conteudo.append(Spacer(1, 2 * mm))
        resumo_primeira_pagina_conteudo.append(
            criar_box_aviso(
                " | ".join(alertas[:3]),
                fundo="#EDF7FF",
                borda_box="#CAE4F7",
    ...
    resumo_primeira_pagina = Table([[resumo_primeira_pagina_conteudo]], colWidths=[doc.width])
    resumo_primeira_pagina.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), colors.white),
        ("BOX", (0, 0), (-1, -1), 0.7, cor_borda),
        ("LEFTPADDING", (0, 0), (-1, -1), 12),
        ("RIGHTPADDING", (0, 0), (-1, -1), 12),
    ...
    capa_primeira_pagina = criar_ficha_imovel(
        destaque,
        1,
        max(len(linhas), 1),
        badge_texto=status_hero,
    )[0]

    historia: list[Any] = [capa_primeira_pagina, Spacer(1, 4 * mm), resumo_primeira_pagina]
    ...
    if not linhas:
        ...
        historia.extend(montar_secao("Carteira completa", f"Lista de imoveis ({_formatar_inteiro(len(linhas))} unidades)"))

        tabela_linhas: list[list[Any]] = [[
            paragrafo_seguro("Imovel", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Local", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Tipo", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Status", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Valor", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Garantido", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Entrega", estilo_tabela_head, preservar_quebras=False),
            paragrafo_seguro("Reserva", estilo_tabela_head, preservar_quebras=False),
        ]]

        for indice, linha in enumerate(linhas, start=1):
            titulo_compacto = _resumir_texto_exportacao(
                f"{indice}. {_valor_texto_exportacao(linha, {'chave': 'titulo', 'tipo': 'texto'})}",
                42,
            ) or "-"
            local_compacto = _resumir_texto_exportacao(descrever_localizacao(linha), 28) or "-"
            tipo_compacto = _resumir_texto_exportacao(
                _valor_texto_exportacao(linha, {"chave": "tipo_imovel", "tipo": "texto"}),
                16,
            ) or "-"
            status_compacto = _resumir_texto_exportacao(
                _valor_texto_exportacao(linha, {"chave": "status", "tipo": "texto"}),
                12,
            ) or "-"
            reserva_compacta = _resumir_texto_exportacao(
                _valor_texto_exportacao(linha, {"chave": "reserva_cliente", "tipo": "texto"}),
                20,
            ) or "-"

            tabela_linhas.append([
                paragrafo_seguro(titulo_compacto, estilo_tabela_titulo, preservar_quebras=False),
                paragrafo_seguro(local_compacto, estilo_tabela_celula, preservar_quebras=False),
                paragrafo_seguro(tipo_compacto, estilo_tabela_celula, preservar_quebras=False),
                paragrafo_seguro(status_compacto, estilo_tabela_centro, preservar_quebras=False),
                ...
                paragrafo_seguro(descrever_entrega_compacta(linha), estilo_tabela_centro, preservar_quebras=False),
                paragrafo_seguro(
                    reserva_compacta,
                    estilo_reserva if reserva_compacta != "-" else estilo_tabela_celula,
                    preservar_quebras=False,
                ),
            ])
        ...
        tabela = LongTable(
            tabela_linhas,
            colWidths=[44 * mm, 30 * mm, 18 * mm, 18 * mm, 24 * mm, 24 * mm, 18 * mm, 20 * mm],
            repeatRows=1,
        )
        tabela.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), cor_superficie),
            ("BOX", (0, 0), (-1, -1), 0.65, cor_borda),
            ("INNERGRID", (0, 0), (-1, -1), 0.35, colors.HexColor("#E7EEF5")),
...
def gerar_arquivo_exportacao_imoveis(formato: str, imoveis: list[dict[str, Any]], filtros: dict[str, Any]) -> tuple[bytes, str, str]:
    ...
    if formato_normalizado not in {"csv", "xlsx", "pdf"}:
        raise HTTPException(status_code=400, detail="Formato de exportacao inválido. Use csv, xlsx ou pdf.")
...
def _normalizar_payload_evolucao_obra(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    data_referencia = _valor_chave(payload, "data_referencia", "data_referência") or date.today()
    observacoes = _texto_limpo(_valor_chave(payload, "observacoes", "observações"))[:500] or None
    identificadores_imoveis_brutos = payload.get("identificadores_imoveis") or []
    if not isinstance(identificadores_imoveis_brutos, (list, tuple, set)):
        identificadores_imoveis_brutos = []

    identificadores_imoveis: list[str] = []
    for valor in identificadores_imoveis_brutos:
        identificador = str(valor or "").strip()
        if identificador and identificador not in identificadores_imoveis:
            identificadores_imoveis.append(identificador)

    return {
        "percentual_conclusao_obra": percentual,
        "data_referencia": data_referencia,
        "observacoes": observacoes,
        "identificadores_imoveis": identificadores_imoveis,
    }
...
def _dados_endereco_lote(registro: dict[str, Any]) -> dict[str, str]:
    return {
        "base": _base_endereco_para_lote(_valor_chave(registro, "endereco", "endereço")),
...
def _mesmo_endereco_lote(referência: dict[str, str], candidato: dict[str, Any]) -> bool:
    dados = _dados_endereco_lote(candidato)
    if not referência["base"] or dados["base"] != referência["base"]:
        return False
    for chave in ("cidade", "bairro", "estado"):
        if referência[chave] and dados[chave] and referência[chave] != dados[chave]:
            return False
    if referência["cep"] and dados["cep"] and referência["cep"] != dados["cep"]:
        return False
    return True
...
async def registrar_evolucao_obra_lote_mesmo_endereco(
    ...
    *,
    ...
    referência = await buscar_imovel_por_id(conexao, esquema, identificador_imovel)
    if not referência:
        return None

    dados_referência = _dados_endereco_lote(referência)
    if not dados_referência["base"]:
        raise HTTPException(
            status_code=400,
            detail="Este imovel não possui endereço suficiente para atualizacao em lote.",
        )
    ...
    candidatos = await listar_imoveis_candidatos_mesmo_endereco(
        conexao,
        esquema,
        cep_normalizado=dados_referência["cep"],
        cidade=referência.get("cidade"),
        bairro=referência.get("bairro"),
        estado=referência.get("estado"),
    )

    alvos = [item for item in candidatos if _mesmo_endereco_lote(dados_referência, item)]
    if not any(str(item.get("identificador_imovel")) == str(identificador_imovel) for item in alvos):
        alvos.append(referência)

    identificadores_selecionados = set(dados.get("identificadores_imoveis") or [])
    if identificadores_selecionados:
        alvos = [
            item for item in alvos
            if str(item.get("identificador_imovel") or "") in identificadores_selecionados
        ]
        if not alvos:
            raise HTTPException(
                status_code=400,
                detail="Selecione ao menos uma unidade válida do lote para aplicar a evolução da obra.",
            )

    registros = []
    imoveis_atualizados = []
    imovel_referência_atualizado = None
    for alvo in alvos:
        ...
        if imovel:
            ...
            if alvo_id == str(identificador_imovel):
                imovel_referência_atualizado = imovel

    return {
        "registros": registros,
        "imoveis": imoveis_atualizados,
        "imovel_referência": imovel_referência_atualizado or referência,
...
async def atualizar_imovel(
    ...
    historico_criado = await _registrar_evolucao_obra_se_necessario(
        ...
        dados_normalizados["percentual_conclusao_obra"],
        registrado_por=registrado_por,
        observacoes="Atualizacao pelo cadastro do imóvel.",
    )
    if historico_criado:
...
def _primeiro_texto_preenchido(*valores: Any) -> str:
    for valor in valores:
        texto = _limpar_texto_importacao(valor)
        if texto:
            return texto
    return ""


def _limpar_texto_importacao(valor: Any) -> str:
...
def _gerar_descricao_importacao(
    *,
    ...
    pares = [
        ...
        ("área real", f"{_formatar_medida(area_real)} m2" if valor_planilha_como_texto(area_real) else ""),
        (
            "área equivalente",
            f"{_formatar_medida(area_equivalente)} m2" if valor_planilha_como_texto(area_equivalente) else "",
        ),
        ("área total construida", f"{_formatar_medida(area_total)} m2" if valor_planilha_como_texto(area_total) else ""),
    ]
...
def _montar_payload_importacao_linha(
    ...
    descricao_manual = _primeiro_texto_preenchido(obter_valor_coluna_planilha(linha, colunas, "descrição"))
    ...
    endereco = _primeiro_texto_preenchido(
        obter_valor_coluna_planilha(linha, colunas, "endereço"),
    ...
    valor_desconto_minimo = _primeiro_valor_preenchido(
        obter_valor_coluna_planilha(linha, colunas, "valor_desconto_minimo"),
        contexto.get("valor_desconto_minimo_padrao"),
    )
    valor_desconto_maximo = _primeiro_valor_preenchido(
        obter_valor_coluna_planilha(linha, colunas, "valor_desconto_maximo"),
        contexto.get("valor_desconto_maximo_padrao"),
    )
    ...
    payload = normalizar_payload_imovel(
        {
            ...
            "area_m2": normalizar_decimal(area_m2, "área em m2"),
...
def carregar_planilha_imoveis(
    *,
    ...
    if load_workbook is None:
        raise HTTPException(
            status_code=500,
            detail="A dependência openpyxl não está disponivel para processar planilhas Excel.",
        )
    ...
    if extensao not in EXTENSOES_PLANILHA_IMPORTACAO:
        raise HTTPException(status_code=400, detail="Envie uma planilha Excel válida nos formatos .xlsx ou .xlsm.")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Não foi possível ler a planilha Excel enviada.") from erro

    try:
        if not workbook.sheetnames:
            raise HTTPException(status_code=400, detail="A planilha enviada não possui abas disponíveis.")
        ...
        if not cabecalhos or linha_planilha_vazia(cabecalhos):
            raise HTTPException(status_code=400, detail="A planilha enviada não possui cabecalho válido.")
        ...
        if total_linhas <= 0:
            raise HTTPException(status_code=400, detail="A planilha enviada não possui linhas de dados para importacao.")
    ...
    for item in planilha["itens"]:
        ...
        try:
            async with conexao.transaction():
                ...
                if existente:
                    registro = await atualizar_imovel_repositorio(
                        conexao,
                        esquema,
                        str(existente["identificador_imovel"]),
                        payload,
                    )
                    await _registrar_evolucao_obra_se_necessario(
                        conexao,
                        ...
                        registrado_por=registrado_por,
                        observacoes="Atualizacao via importacao em lote.",
                    )
                    atualizados.append(serializar_imovel(registro))
                    if chave:
...
async def salvar_midias_do_imovel(
    ...
    *,
    ...
    if not arquivos:
        raise HTTPException(status_code=400, detail="Nenhuma mídia foi enviada.")
