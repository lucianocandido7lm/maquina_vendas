...
PERMISSOES_GESTAO_CADASTRO_IMOVEIS = (
...
async def _exigir_gestao_cadastro_imoveis(
    conexao,
    identificador_usuario: str,
    detalhe: str,
) -> None:
...
async def exportar_imoveis(
    ...
    async with pool.acquire() as conexao:
        await exigir_permissao_portal(
            ...
            identificador_usuario,
            "imoveis.view",
            "Você não tem permissão para exportar imóveis.",
        )
...
async def geocodificar_localizacao_imovel(
    request: Request,
    q: str = Query("", description="Consulta textual para localizar o imóvel"),
    usuario=Depends(obter_usuario_autenticado),
):
    consulta = " ".join(str(q or "").split())
    if len(consulta) < 3:
        raise HTTPException(status_code=400, detail="Informe uma localização válida para consultar o mapa.")
    ...
    async with pool.acquire() as conexao:
        await _exigir_qualquer_permissao(
            ...
            "Você não tem permissão para consultar a localização deste imóvel.",
        )
...
async def buscar_imovel(
    ...
    async with pool.acquire() as conexao:
        await _exigir_qualquer_permissao(
            ...
            identificador_usuario,
            ("imoveis.view", "imoveis.edit", "imoveis.media.manage"),
            "Você não tem permissão para acessar os dados deste imóvel.",
        )
...
async def listar_historico_evolucao_obra(
    ...
    async with pool.acquire() as conexao:
        await _exigir_qualquer_permissao(
            ...
            identificador_usuario,
            ("imoveis.view", "imoveis.edit", "imoveis.media.manage"),
            "Você não tem permissão para acessar a evolução da obra deste imóvel.",
        )
...
async def registrar_evolucao_obra(
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem registrar a evolucao da obra.",
        )
    ...
    return {
        "mensagem": "Evolução da obra registrada com sucesso.",
        "evolucao": serializar_evolucao_obra(evolucao),
        "histórico": [serializar_evolucao_obra(item) for item in historico],
...
async def registrar_evolucao_obra_lote_endereco(
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem registrar a evolucao da obra em lote.",
        )
    ...
    selecionados = payload.identificadores_imoveis or []
    mensagem = (
        f"Evolucao aplicada em {total} imovel(is) selecionado(s) do mesmo endereço."
        if selecionados
        else f"Evolucao aplicada em {total} imovel(is) do mesmo endereço."
    )
    return {
        "mensagem": mensagem,
        ...
        "histórico": [serializar_evolucao_obra(item) for item in historico],
        "item": serializar_imovel(resultado["imovel_referência"], midias=midias, evolucao_obra=historico),
    }
...
async def criar_imovel(
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem cadastrar imoveis.",
        )
...
async def importar_imoveis(
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem importar imoveis em lote.",
        )
    ...
    mensagem = (
        f"Importacao concluida com {resumo['total_importados']} imovel(is) importado(s) "
        f"e {resumo['total_atualizados']} atualizado(s)."
    )
...
async def atualizar_imovel(
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem editar imoveis.",
        )
...
async def remover_imovel(
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem excluir imoveis.",
        )
...
async def upload_midias(
    ...
    if len(arquivos) > IMOVEIS_TOTAL_MAXIMO_ARQUIVOS_POR_ENVIO:
        raise HTTPException(
            status_code=400,
            detail=f"Envie no máximo {IMOVEIS_TOTAL_MAXIMO_ARQUIVOS_POR_ENVIO} arquivos por vez.",
        )
    ...
    async with pool.acquire() as conexao:
        await _exigir_gestao_cadastro_imoveis(
            conexao,
            identificador_usuario,
            "Somente gestores e administradores podem enviar ou remover midias dos imoveis.",
        )
    ...
    return {
        "mensagem": "Mídias enviadas com sucesso.",
