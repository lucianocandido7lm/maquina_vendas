(function () {
  ...
  function renderPessoas(payload) {
    ...
    if (!items.length) {
      renderEmpty(el.pessoas, "Nenhum usuário online agora", "Não houve atividade de usuários dentro da janela selecionada.");
      return;
    }

    el.pessoas.innerHTML = items.map((item) => {
      const nome = describeUser(item);
      const sessoesUsuario = number(item.sessoes_usuario_janela);
      const meta = [
        item.correio_eletronico,
        item.matricula ? `Matrícula ${item.matricula}` : "",
        sessoesUsuario > 1 ? `${formatNumber(sessoesUsuario)} sessões recentes` : ""
      ].filter(Boolean).join(" • ");
      ...
      return `
        <section class="tl-monitor-person">
          <div class="tl-monitor-person__identity">
            <div class="tl-monitor-avatar">${escapeHtml(initials(nome))}</div>
