-- KPI oficial para validacao no Postgres (Backend)
-- Parametros esperados no cliente SQL: :start_date e :end_date
  ...
  SELECT
    ...
    (
      ...
      WHERE LOWER(COALESCE(h.ativo_negocio, '')) = 's'
        AND (h.data_inicio_vigencia IS NULL OR h.data_inicio_vigencia <= :end_date::date)
