...
> **Atualização 23/06/2026:** incluídas as regras oficiais de classificação de SLA e de restrição do lead, utilizadas nos filtros e na sinalização da Aba 2.
...
Situação
Restrição
SLA / classificação de SLA e classificação de restrição, quando aplicável ao detalhe
```
...
| `.SLA` | valor de SLA individual já homologado no Power BI |
| `.SLA Classificação` | dentro/expirado conforme limite da situação |
| `restricao_lead` | valor original de restrição do lead |
| `.SLA Classificação Restrição` | faixa operacional da restrição |
...
## 8.13 `.SLA Classificação` — regra operacional de prazo por situação

Esta é uma **classificação por registro do lead**, usada principalmente nos filtros e no detalhamento operacional da Aba 2. Ela não deve ser confundida com a medida `[SLA - Médio]` da Aba 3:

- `[SLA - Médio]` mede a média de tempo entre etapas do funil, de forma agregada;
- `.SLA Classificação` avalia se o SLA do registro individual está dentro ou fora do limite previsto para sua situação.

### Campo-base obrigatório

A regra depende do campo abaixo já existente na camada de dados:

```text
fato_leads_comercial[.SLA]
```

O cálculo de origem do campo `.SLA` não foi disponibilizado neste documento. Portanto, a Máquina de Vendas deve **reutilizar a mesma regra/campo validado no Power BI**, sem recriar uma interpretação alternativa de prazo.

> Não substituir `.SLA` por diferença simples entre datas sem homologação. O campo pode possuir regras próprias de dias úteis, pausas, status ou datas de início/fim.

### Limites oficiais por situação

| `lead_situacao` | Limite de SLA | Resultado esperado |
|---|---:|---|
| `Lead` | 3 | Dentro do SLA até 3; expirado acima de 3 |
| `Atendimento - SDR` | 3 | Dentro do SLA até 3; expirado acima de 3 |
| `Atendimento - IA` | 3 | Dentro do SLA até 3; expirado acima de 3 |
| `Agendamento` | 7 | Dentro do SLA até 7; expirado acima de 7 |
| `Repescagem` | 90 | Dentro do SLA até 90; expirado acima de 90 |
| `Visita` | 7 | Dentro do SLA até 7; expirado acima de 7 |
| `Base` | 5 | Dentro do SLA até 5; expirado acima de 5 |
| `Tratativa` | 30 | Dentro do SLA até 30; expirado acima de 30 |
| Qualquer outra situação | — | Sem classificação (`BLANK`/`NULL`) |
...
VAR Limite =
    SWITCH(
        TRUE(),
        'fato_leads_comercial'[lead_situacao] = "Lead", 3,
        'fato_leads_comercial'[lead_situacao] = "Atendimento - SDR", 3,
        'fato_leads_comercial'[lead_situacao] = "Atendimento - IA", 3,
        'fato_leads_comercial'[lead_situacao] = "Agendamento", 7,
        'fato_leads_comercial'[lead_situacao] = "Repescagem", 90,
        'fato_leads_comercial'[lead_situacao] = "Visita", 7,
        'fato_leads_comercial'[lead_situacao] = "Base", 5,
        'fato_leads_comercial'[lead_situacao] = "Tratativa", 30,
        BLANK()
    )
RETURN
IF(
    ...
    BLANK(),
    IF(
        'fato_leads_comercial'[.SLA] > Limite,
        "SLA Expirado",
        "Dentro do SLA"
    )
)
...
### Regra de implementação fora do Power BI

Implementar com a mesma semântica:

```text
se situacao não estiver no mapa de limites:
    sla_classificacao = null
senão se sla > limite_da_situacao:
    sla_classificacao = "SLA Expirado"
senão:
    sla_classificacao = "Dentro do SLA"
```

Regras importantes:

1. O comparativo é estritamente `>`; portanto, um SLA exatamente igual ao limite continua como `Dentro do SLA`.
2. Situações não mapeadas não podem ser forçadas para “Dentro do SLA” nem “SLA Expirado”. Retornar `NULL`/vazio.
3. O filtro da tela deve oferecer somente os valores com significado de negócio:
...
4. Ao selecionar uma classificação, ela deve filtrar **antes** da paginação da tabela e refletir também nos históricos carregados para as linhas selecionadas.
5. A classificação deve ser calculada sobre a situação efetivamente exibida no registro operacional do detalhe. Não aplicar a classificação da situação atual em um evento histórico de outra situação.

### Uso visual recomendado

| Situação | Exibição |
|---|---|
| `Dentro do SLA` | tag/ícone verde discreto |
| `SLA Expirado` | tag/ícone vermelho ou laranja de atenção |
| `NULL` / sem regra | vazio ou `—`; não sinalizar como falha |

---

## 8.14 `.SLA Classificação Restrição` — faixa de restrição do lead

Esta é uma classificação por registro do campo:

```text
fato_leads_comercial[restricao_lead]
```

Ela deve ser utilizada como filtro operacional e coluna de auditoria na Aba 2. Não altera a quantidade das etapas do funil, exceto quando o usuário aplicar explicitamente esse filtro.

### Faixas oficiais

| Condição de `restricao_lead` | Classificação |
|---|---|
| entre `100` e `300`, inclusive | `Restrição Baixa` |
| igual a `0` ou nulo | `Sem Restrição` |
| qualquer outro valor | `Restrição Acima` |

> A faixa `1` a `99` é classificada como `Restrição Acima`, pois não se enquadra na faixa baixa e também não é zero/nula. Não alterar esta regra sem validação de negócio.

### DAX de referência

```dax
.SLA Classificação Restrição = 
    SWITCH(
        TRUE(),
        'fato_leads_comercial'[restricao_lead] >= 100 && 'fato_leads_comercial'[restricao_lead] <= 300, "Restrição Baixa",
        'fato_leads_comercial'[restricao_lead] = 0 || ISBLANK('fato_leads_comercial'[restricao_lead]), "Sem Restrição",
        "Restrição Acima"
    )
```
...
```text
se restricao_lead é nula ou igual a 0:
    classificacao_restricao = "Sem Restrição"
senão se restricao_lead está entre 100 e 300, inclusive:
    classificacao_restricao = "Restrição Baixa"
senão:
    classificacao_restricao = "Restrição Acima"
```

### Normalização obrigatória antes da classificação

Caso a fonte armazene restrição como texto, normalizar antes de avaliar:
...
Não transformar valores inválidos silenciosamente em zero. Valor não numérico deve ser registrado para correção de qualidade de dados; até a definição de regra específica, tratá-lo como `Restrição Acima` somente se o Power BI também o fizer após conversão.

### Uso no detalhe

O filtro **Restrição** deve apresentar exatamente:
...
A coluna pode ser exibida como `Classificação de Restrição`, com o valor numérico original de `restricao_lead` disponível no painel expandido ou tooltip de auditoria.

---

## 8.15 Validação das classificações de SLA e restrição

Antes de publicar, validar uma amostra que cubra todas as condições:

| Cenário | Entrada | Saída esperada |
|---|---|---|
| SLA no limite | `lead_situacao = Lead`, `.SLA = 3` | `Dentro do SLA` |
| SLA acima do limite | `lead_situacao = Lead`, `.SLA = 3,01` | `SLA Expirado` |
| situação sem mapa | `lead_situacao = Proposta` | vazio/`NULL` |
| SLA nulo com situação mapeada | `lead_situacao = Visita`, `.SLA = NULL` | conforme comportamento homologado do Power BI; em DAX atual resulta `Dentro do SLA` porque `BLANK() > 7` é falso |
| restrição nula | `restricao_lead = NULL` | `Sem Restrição` |
| restrição zero | `restricao_lead = 0` | `Sem Restrição` |
| limite inferior baixo | `restricao_lead = 100` | `Restrição Baixa` |
| limite superior baixo | `restricao_lead = 300` | `Restrição Baixa` |
| valor entre 1 e 99 | `restricao_lead = 50` | `Restrição Acima` |
| valor acima da faixa | `restricao_lead = 301` | `Restrição Acima` |

> **Ponto de atenção:** a fórmula DAX fornecida classifica `.SLA` nulo de uma situação mapeada como `Dentro do SLA`, porque a comparação `BLANK() > Limite` retorna falso. A implementação deve manter esse comportamento para ficar idêntica ao Power BI. Caso o negócio queira “Sem SLA” como terceira categoria, isso precisa ser uma alteração homologada de regra, não uma interpretação do agente.
...
SLA
SLA Classificação
Restrição (Classificação de Restrição)
Idlead
Situação
Agentes
Mídia
...
A regra completa, limites por situação, exceções e comportamento para valores nulos estão definidos nas seções **8.13 a 8.15**. Não criar uma classificação diferente nesta tela.
